...
- **首期限定范围**：首期只对 ai-agent-app 对应 Realm 开放，用真实行为数据（选择率、完成率、放弃率）决定是否向其他 Realm 推广。
...
- **首期范围边界**：首期仅对 ai-agent-app 对应 Realm 开放；不默认向所有 Realm 开启自动注册。推广由首期观测数据决定。
...
- **验证码错误**：提示"验证码错误或已失效"，允许重新输入；达到连续错误上限后该次验证码作废，需重新发送。
- **验证码过期**：提示"验证码已失效"，需重新发送。
- **发送频率受限**：提示"请稍后再试"，不发送新验证码。
