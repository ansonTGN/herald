...
- `docs/user-stories/billing/points-free-user.md` — US-FU-001 ~ US-FU-003：注册/免费积分进入被指定为注册接收账户的积分账户（账户级配置，非默认账户）
...
- Realm 级积分账户目录：创建、编辑、启用/禁用、展示顺序、注册积分接收账户标记
...
- 注册积分接收账户：Realm 内可指定一个积分账户为注册/免费定期系统发放积分的接收账户（积分账户自身配置，**非默认账户**，不自动覆盖全部应用）；SDK/管理员主动发放需显式指定目标账户，无隐式池解析
...
- 所有积分都归属到账户池；注册/免费定期系统发放积分归入被指定为注册接收账户的积分账户，SDK/管理员主动发放按显式指定的目标账户入账。
...
- **统一池模型**：一切积分皆为池；注册/免费定期积分入注册接收账户，主动发放按显式账户入账，无隐式池解析
...
- 每个积分账户有名称、可选展示顺序、启用状态、是否为注册积分接收账户
- 同一 Realm 同时只有一个注册积分接收账户
...
- 注册初始积分（registration_credit）、免费定期积分（free_periodic_credit）：进入该 Realm 注册积分接收账户
...
- 支持在 Realm 内管理积分账户目录：创建、编辑名称与展示顺序、启用/禁用、标记注册积分接收账户
- 支持为积分账户配置覆盖的 Client App 集合（至少一个；注册接收账户也需显式配置覆盖集，不自动覆盖全部应用）
...
- 支持注册/免费定期系统发放积分进入注册积分接收账户；SDK/管理员主动发放显式指定目标账户
...
- Realm 内可定义多个积分账户，可配置覆盖应用、归属套餐、注册积分接收账户标记
...
- 注册/免费定期积分进入注册积分接收账户；主动发放按显式账户入账；注册接收账户覆盖集显式配置，不自动覆盖全部应用
...
- 管理入口：Realm Admin 可在管理后台访问积分账户管理页（积分账户列表、覆盖应用、归属套餐/积分包、积分策略、启用状态、注册积分接收账户标记）
...
- 注册/免费定期积分归入该 Realm 的注册积分接收账户（每 Realm 一个，账户级配置）；主动发放按显式账户入账
- 每笔积分发放显式指定目标账户（履约→Mapping、订阅→订阅绑定账户、SDK/管理员→请求 `bucketId`、注册/免费→注册接收账户），无隐式池解析
