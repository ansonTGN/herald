...
- **Webhook 补偿**：续费预期预发与正式 webhook 发放的幂等对齐依赖 webhook 补偿框架（见 `docs/prd/billing/webhook-compensation.md`）
...
- 预发后续费失败/取消时回收预发积分：未到生效时间或未被消费的预发积分直接作废、不计入用户可用余额（无需反向调整钱包）；已被部分消费的预发积分按既有退款回收与负债记录处理。回收幂等，重复事件不重复回收
- 账本与交易记录同时保留积分的实际发放时间与预期生效时间，供管理员/审计对账区分二者；这些时序字段不对普通用户暴露
