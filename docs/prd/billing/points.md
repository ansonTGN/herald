...
- **Webhook 补偿**：订阅生命周期事件驱动的配额权益授予/撤销沿用既有 webhook 幂等键与补偿框架（见 `docs/prd/billing/webhook-compensation.md`）
...
- 配额定义归属 entitlement mapping（订阅）与 realm default config（免费周期），**不在 bucket 上**配置
