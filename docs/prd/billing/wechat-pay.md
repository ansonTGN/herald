...
- **配置管理规则**：每个 Realm 可配置一个微信支付商户号；配置项包括 App ID、Merchant ID、商户 RSA 私钥（PEM 格式）、证书序列号、API v3 Key（32 字节）、Notify URL（必须 HTTPS）；敏感信息（私钥、API v3 Key）应加密存储（当前实现为明文存储，见实现差异 BUG-005）；不允许修改平台类型（创建后不可变）；编辑时敏感字段留空则保留现有值
...
- **安全加密**：私钥和 API v3 Key 应加密存储（当前实现为明文写入数据库，见实现差异 BUG-005）；回调数据解密验证（当前解密算法与微信规范不符，见实现差异 BUG-002）
...
- **PRD 规范**：private_key 和 v3_key 加密存储
- **当前实现**：private_key 和 v3_key 明文写入 `realm_config` 表（`config_value` 字段），仅通过 `is_secret` 标记和 API 响应脱敏
- **影响**：数据库泄露时直接暴露商户私钥和 API 密钥
- **代码位置**：`wechat_config_handlers.rs` → `save_wechat_config()` / `update_wechat_config_internal()`
- **修复方向**：入库前使用应用层加密（如 AES-256-GCM），读取时解密；或使用 Vault/KMS 等密钥管理服务
- **状态**：待修复
