...
- 每次对账运行输出统计日志（拉取数、补处理成功数、失败数）
- 补偿处理失败时写 Error 日志（含 realm_id、event_id、错误原因）
...
- provider 侧存在且 Herald 已存在 → 跳过（幂等，复用 webhook 处理的 DB 层支付事件记录去重）
- 不单独实现"provider 与 Herald 状态不一致"对比分支；状态收敛依赖对缺失事件重放 webhook 业务逻辑（与 §8"无自动修复"决策一致），补偿处理本身失败时写 Error 日志
...
**日志规则**：
- 每次对账运行记录统计日志：拉取事件数、补处理成功数、补处理失败数
- 补偿处理失败时写 Error 日志，包含 realm_id、event_id、错误原因
