...
- 平台证书用于 WeChat 回调验签；证书缺失或过期导致验签失败时，回调被拒绝并产生可观测诊断
...
- 回调处理失败不自动重试，依赖微信自身重发策略（与现有 Stripe/Creem 一致）
- 回调事件纳入既有 Webhook 补偿入口，支持人工重放；重放同样幂等
...
- 系统在证书接近过期时自动重新下载替换
- 证书异常（缺失、临期、验签失败）产生可观测诊断，便于运维；手工配置的平台公钥可作为验签覆盖兜底
