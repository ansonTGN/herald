...
- 以下操作类别的审计覆盖：
  ...
  - **认证事件**：用户登录、登出、登录失败
  - **计费事件**：积分授予、套餐订阅、支付等计费相关操作
  - **OAuth 事件**：Client App 管理、授权码签发、Token 签发/撤销
...
- 审计日志查询仅 Realm Admin 和 Admin Realm 管理员可访问
...
- 页面入口：管理后台新增审计日志页面，仅对 Realm Admin 和 Admin Realm 管理员可见
