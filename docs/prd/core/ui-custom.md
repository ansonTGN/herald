...
- **权限要求**：仅 Realm Admin（持有 `settings.manage`）可查看和修改品牌资产配置；Regular User 仅作为配置的消费方在 auth 流页面看到品牌化呈现，无配置入口
...
- **访问控制原则**：管理端读写要求 Realm Admin（`settings.manage` 权限）并通过 Realm 归属校验；公共配置读取为无认证读取（与现有 public 配置通道一致，供未登录终端用户在 auth 流页面加载品牌资产）
