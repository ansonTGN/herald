...
- `[US-CD-003]` 自定义域名配置的草稿与发布 (P1)，来源 `docs/user-stories/core/realm-custom-domain.md`
  - 角色：Realm Admin
  - 摘要：自定义域名配置遵循草稿/发布/恢复生命周期，未发布前不影响已生效的自定义域名
...
- 域名全局唯一约束（同一精确域名不可被多个 Realm 同时占用，跨 Realm 草稿与已发布均占位）
...
- 自定义域名配置的草稿/发布/恢复生命周期（保存草稿、放弃草稿、发布、恢复上一版）
- 域名注册与证书授权的统一映射（已发布生效的自定义域名映射到对应 Realm）
...
- **White-label（已上线）** — 自定义域名的配置生命周期沿用 white-label 已发布的 draft/publish/restore 范式
...
让 Herald 租户能够在本 Realm 配置一个完全自有的品牌登录域名（如 `login.acme.com`），为后续在该自有品牌域名下承载 auth 流提供配置与证书授权基础。本期交付配置能力与证书滥用防护：Realm Admin 配置自定义域名、获取 CNAME 指引、查看生效状态、遵循草稿/发布/恢复生命周期；Herald 仅对已注册并生效的域名授权反代层签发 TLS 证书，未注册域名不被授权签发，防止证书滥用与钓鱼。
...
- 配置沿用与 white-label 同构的草稿/发布/恢复生命周期
...
- **域名全局唯一**：同一精确域名不可被多个 Realm 同时占用；跨 Realm 的草稿与已发布均占位（避免两 Realm 草稿撞车后发布冲突）；后配置的 Realm 在保存草稿时被拒绝并提示域名已被占用
...
- **未授权域名拒绝**：Herald 仅对已在某 Realm 注册（经认证 Realm Admin 操作）并已发布生效的域名授权签发 TLS；未注册域名不授权签发证书，防止证书滥用与钓鱼
- **生效模型**：Realm Admin 保存自定义域名草稿时不影响终端用户当前已生效的自定义域名配置；发布后才更新生效的自定义域名；至少保留上一版用于快速恢复
- **发布副作用**：发布自定义域名配置时，系统把该 hostname 写入域名注册映射（供证书授权门控查询）；恢复上一版时同步回滚该映射
- **证书授权门控查询基准**：证书授权门控仅以"已发布且启用"为生效判定；CNAME 是否已验证、TLS 是否已就绪是面向 Realm Admin 的展示态信息，不纳入授权判定（否则授权与签发形成循环）
...
- **草稿未发布**：存在未发布自定义域名草稿时，当前已发布生效的自定义域名配置不受影响；草稿仅管理端可见
- **恢复上一版**：发布错误自定义域名配置后，Realm Admin 可恢复上一版，系统同步回滚域名注册映射
...
- **草稿与发布**：自定义域名配置沿用与 white-label 同构的 draft/publish/restore 范式；只有已发布配置影响当前生效的自定义域名
- **错误配置恢复**：Realm Admin 可将已发布自定义域名配置恢复到上一版
- **未授权域名拒绝**：未注册域名不被授权签发证书
- **配置隔离与权限**：自定义域名按 Realm 隔离，仅 Realm Admin 可配置

### 5.2 验收目标

- Realm Admin 能在管理后台 Settings 页面为本 Realm 成功配置并保存自定义登录域名草稿，域名被全局唯一约束保护
- Realm Admin 能看到 CNAME 指引与域名生效状态（CNAME / TLS 是否就绪）
- 尝试配置已被其他 Realm 占用的域名时，系统拒绝并提示域名已被占用
- Realm Admin 保存未发布草稿期间，当前已发布生效的自定义域名配置不受影响
- 自定义域名发布后，该 hostname 被写入域名注册映射；证书授权门控据此授权
- 发布错误自定义域名配置后，Realm Admin 能恢复上一版
...
- **接口能力范围**：
  - 管理端：自定义域名配置的读取、草稿保存、放弃草稿、发布、恢复上一版（Realm Admin），沿用现有 Realm Config 管理端模式
...
- **配置表单**：包含自定义域名输入字段，支持保存草稿、放弃草稿、发布和恢复上一版（沿用 white-label 范式）
...
- **草稿提示**：存在未发布草稿或表单有未保存改动时显示草稿提示（复用 white-label 范式）
...
- **自定义域名配置沿用 white-label 配置范式**：自定义域名配置沿用与 white-label 同构的 draft/publish/restore 生命周期（保存草稿、放弃草稿、发布、恢复上一版）。实现层结构由 /t-design 定义，不在 PRD 范围。
- **hostname 全局唯一**：域名注册映射以 hostname 为全局唯一键，保证同一精确域名不被多个 Realm 同时占用；存储与表结构细节由技术设计定义，不在 PRD 范围
- **证书授权门控查询基准**：授权判定仅以"已发布且启用"为准；CNAME/TLS 状态为展示态字段，不纳入授权判定，避免授权与签发形成循环
