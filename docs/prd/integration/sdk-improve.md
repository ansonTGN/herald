...
- Realm 创建需额外校验 API Key 是否具备平台级权限
...
- **Realm 创建特权**：创建 Realm 的端点需校验 API Key 具备平台级权限，普通 Realm 的 API Key 不可创建 Realm
- 复用现有 `handle_response` 统一错误处理模式

### 接口能力边界

- Realm：创建、列表、详情
- User：创建、列表、详情（限本 Realm）
- Client App：创建、列表、详情（限本 Realm）
