...
- API Key 自身作为 Principal 参与授权，以自身唯一标识作为 Principal ID。
- 本文件中的 SDK 资源管理能力均要求 API Key Principal 具备对应 RBAC 权限。
- API Key 仍受 Realm 隔离约束，只能操作其所属 Realm 的资源。
- 创建 Realm 额外要求 API Key Principal 属于 admin realm 且具备 `realm:create`。
...
**场景 1：创建 Realm 成功**
```gherkin
Given SDK 已使用 admin realm 的 API Key 初始化，且该 API Key Principal 具备 `realm:create`
...
**场景 2：查询 Realm 列表**
```gherkin
Given SDK 已使用 API Key 初始化，且该 API Key Principal 具备 `realm:list`
And 系统中存在多个可见 Realm
...
**场景 3：查询 Realm 详情**
```gherkin
Given SDK 已使用 API Key 初始化，且该 API Key Principal 具备 `realm:view`
...
**场景 4：创建 Realm 权限不足**
```gherkin
Given SDK 使用的 API Key Principal 缺少 `realm:create` 或不具备 admin realm 创建权限
...
**场景 1：创建用户成功**
```gherkin
Given SDK 已使用 API Key 初始化，且该 API Key Principal 具备目标 Realm 的 `users:create`
...
**场景 2：查询用户列表**
```gherkin
Given SDK 已使用 API Key 初始化，且该 API Key Principal 具备目标 Realm 的 `users:view`
...
**场景 5：跨 Realm 拒绝**
```gherkin
Given API Key Principal 具备 Realm-A 的用户管理权限
...
**场景 6：缺少管理权限**
```gherkin
Given SDK 使用的 API Key Principal 缺少 `users:create`
...
```gherkin
Given SDK 已使用 API Key 初始化，且该 API Key Principal 具备目标 Realm 的 `clients:create`
...
```gherkin
Given SDK 已使用 API Key 初始化，且该 API Key Principal 具备目标 Realm 的 `clients:view`
...
**场景 6：缺少管理权限**
```gherkin
Given SDK 使用的 API Key Principal 缺少 `clients:create`
