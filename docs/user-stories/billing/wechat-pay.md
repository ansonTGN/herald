...
**【用户故事】**
**作为**：Regular User
**我希望**：在购买页面选择微信支付后扫码完成付款
**从而**：使用微信便捷地购买订阅套餐或积分包
...
Then 页面显示 "支付成功"
And 如果是订阅套餐，页面显示我的订阅信息
And 如果是积分包，页面显示发放的积分数和新的积分余额
```
...
Given 用户已创建微信支付订单
When 前端发起支付状态查询请求
Then 如果支付成功，返回支付成功状态
And 如果支付未完成，返回待支付状态
And 如果订单已关闭，返回已关闭状态
```

**场景 2：回调未到达时的补偿查询（需要 Worker 定时任务支持）**
```gherkin
Given 用户已创建微信支付订单
And 订单创建超过 5 分钟且未收到回调通知
When 系统执行定时补偿查询
Then 如果实际已支付，更新本地订单状态并补发积分
And 如果未支付，保持待支付状态
```
...
**场景 1：自动关闭过期订单（需要 Worker 定时任务支持）**
```gherkin
...
**场景 2：关闭订单前确认未支付（需要 Worker 定时任务支持）**
```gherkin
Given 存在即将关闭的微信支付订单
When 系统准备关闭该订单
...
| 优先级 | 用户故事数量 | 关键故事 |
|--------|------------|---------|
| P0 | 5 | 配置微信支付、查看配置、用户扫码支付、回调处理、主动查询状态 |
