...
And 已存在已启用的积分包 "credits-500"（500 积分，¥50）
And 该积分包已配置微信支付映射
When 我访问积分购买页面
And 我选择积分包 "500 积分包"
And 我点击 "微信支付" 按钮
...
And 已存在已启用的积分包 "credits-1000"（1000 积分，$10）
And 该积分包已配置 Stripe 支付映射
When 我访问积分购买页面
And 我选择积分包 "1000 积分包"
...
**场景 6：个人中心 Points 入口按已启用积分包显示**
```gherkin
Given 我是已登录用户
And 当前 Realm 已存在已启用的积分包
When 我打开个人中心
Then 侧边栏显示 "Points" 菜单
When 当前 Realm 不存在已启用的积分包
...
```gherkin
Given 我是已登录用户
And 当前 Realm 已存在已启用的积分包 "credits-500"
And 该积分包没有已启用的支付平台映射
When 我访问积分购买页面
...
> **注意**：当前后端仅支持按支付平台筛选和分页，时间范围筛选功能待实现。

---

### 故事 3：理解积分包与订阅购买的区别 [US-PU-08]
