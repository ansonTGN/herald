...
And Realm 在首期开放范围内（ai-agent-app 对应 Realm）
...
```gherkin
Given 当前 Realm 未开启自动注册（或不在首期开放范围）
...
1. **首期范围**：首期仅对 ai-agent-app 对应 Realm 开放，不默认向所有 Realm 开启自动注册；用真实行为数据决定是否推广。
