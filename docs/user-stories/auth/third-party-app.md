...
When 第三方应用使用该 API Key 请求权限检查
Then 系统确认认证通过，返回权限检查结果和用户标识
And 系统更新该 API Key 的最后使用时间和使用次数
```
...
When 请求第三方 API
Then 系统返回未授权错误，提示缺少 API Key
```

**场景 4：使用已禁用的 API Key（失败场景）**
```gherkin
Given API Key 已被禁用
When 使用该 API Key 请求第三方 API
Then 系统返回未授权错误，提示 API Key 已禁用
```
...
Given API Key 已过期
When 使用该 API Key 请求第三方 API
Then 系统返回未授权错误，提示 API Key 已过期
```

**场景 6：跨 Realm 访问（失败场景）**
```gherkin
Given API Key 属于 realm-1
When 使用该 API Key 请求 realm-2 的资源
Then 系统返回禁止访问，提示跨 Realm 访问被拒绝
```
...
When 第三方应用使用有效 API Key 查询该客户端应用的订阅状态
Then 系统返回订阅信息，包含 has_subscription=true、status=active、tier=professional 和 plan_name
```
...
When 第三方应用使用有效 API Key 查询该客户端应用的订阅状态
Then 系统返回订阅信息，包含 has_subscription=false、status=none、tier=free
```
...
**场景 5：查询 trial 订阅**
```gherkin
Given 客户端应用的订阅状态为 trialing
When 第三方应用查询该客户端应用的订阅状态
Then 系统返回 has_subscription=true、status=trialing 及对应的 tier 和 plan_name
```
...
## 故事 10：配置 Client App 跳转地址白名单 [US-TP-010]
...
And 点击"Add"按钮
Then 系统提示 URL 格式无效且不添加该地址
```

**场景 3：至少需要一个跳转地址（失败场景）**
```gherkin
Given 管理员在设置页面
...
And 用户完成登录
Then 系统检测到地址不在白名单中，拒绝请求
```

---

## 故事 11：管理 Client App 图标 [US-TP-011]
...
## 故事 12：启用/禁用 Client App [US-TP-012]
...
## 故事 13：配置 Session 有效期策略 [US-TP-013]
...
```gherkin
Given 管理员在 Client App 设置页面
When 管理员设置初始 Session 有效期为 5 分钟
And 设置续期后延长到 2 小时
And 点击保存
...
**场景 4：禁止续期时的提示（失败场景）**
```gherkin
Given Client App 配置为不允许续期
When 用户尝试刷新 Session
Then 系统返回未授权，提示需要重新登录
```

---

## 故事 14：第三方 Web SPA 发起 SSO 登录 [US-TP-014]
