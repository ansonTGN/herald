...
# 2. Claude 自动分析变更
# 3. 运行相关测试
uv run scripts/backend-test.py -- -E 'package(points)'
...
```bash
# 特定测试
uv run scripts/backend-test.py -- test_points_deduction

# 特定包
uv run scripts/backend-test.py -- -E 'package(points)'

# 组合过滤
uv run scripts/backend-test.py -- -E 'package(api) and test(webhook)'

# 排除测试
uv run scripts/backend-test.py -- -E 'not test(slow)'
```
...
- 默认先做定向测试，不直接跑全量 `uv run scripts/backend-test.py`
- 全量测试只在用户明确要求，或影响范围无法可靠界定时才执行
- 生产代码修复优先交给 `backend-dev`，测试层只处理自身拥有的测试问题

## 文件位置

- Skill: `.claude/skills/backend-test-run/SKILL.md`
