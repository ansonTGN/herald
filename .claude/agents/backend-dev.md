...
6. **编译验证**: 运行 `cargo check -p cas-api` 确保通过
7. **质量检查**: 验证代码符合验收标准
...
- [ ] **编译成功**: `cargo check -p cas-api` 返回 0 errors
...
- [ ] **单元测试通过**: `uv run scripts/backend-test.py` 成功
...
- `required` 默认 `true`
- 命令必须使用项目入口（例如 `uv run scripts/backend-test.py -- [filter]`）
