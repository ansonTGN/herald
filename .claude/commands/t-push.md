---
description: 本地运行 CI 检查（backend clippy/fmt、frontend lint/type-check/test、demo lint/type-check），全部通过后 git commit && git push。
allowed-tools:
  - Bash
  - Read
  - Glob
...
若无匹配（例如仅 `.md` 或配置变更），跳过所有检查，直接进入 commit。
...
1. `git add -A`
2. 根据 `git diff --cached --stat` 自动生成简洁的 commit message，格式遵循项目惯例（如 `feat:`, `fix:`, `chore:` 前缀）。
