---
description: 运行单个 Demo 测试文件并按失败类型触发诊断与修复，支持任务状态跟踪与恢复。
...
`AGENTS.md` 是最高约束。本命令执行单文件 Demo 验证和修复编排；若诊断、修复建议、测试证据或代码事实冲突，停止并说明。
...
- 按诊断结果通过 `Agent` tool 分发到对应修复 subagent：`Agent(subagent_type="demo-dev")` / `Agent(subagent_type="frontend-dev")` / `Agent(subagent_type="backend-dev")`。
...
- **在最后输出一行 JSON 结果（用于批量运行解析）**：
  ```json
  Result: {“success”:”true|false”,”test_file”:”demo/e2e/...”,”exit_code”:0|1,”duration”:123.4,”logs”:”test-results/runs/...”,”run_id”:”...”,”fixed”:”true|false”,”error”:”错误信息（如果有）”}
  ```
- Markdown 报告必须包含：
  - `whole_file_result`
  - `fix_attempts`
