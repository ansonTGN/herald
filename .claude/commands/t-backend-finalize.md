---
description: 后端验收后的统一收口命令。固定执行 /simplify、clippy 自动修复、格式化和全量后端测试，并在失败后默认从失败步骤恢复。
argument-hint: [任务名称]
allowed-tools:
  - Read
  - Glob
...
5. 执行 `cargo fmt --all`。
6. 执行全量 `uv run scripts/backend-test.py`。
7. 导出 OpenAPI 文档并生成前端 API 客户端：
   - 执行 `cd backend && cargo run --bin cas-app -- --export-openapi ../frontend/api.json && cd ../`
...
- `cargo fmt --all` 已执行。
- `uv run scripts/backend-test.py` 全量通过。
