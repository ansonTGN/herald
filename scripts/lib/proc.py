...
def kill_process_by_port(port: int) -> bool:
    ...
    if os.name == "nt":
        ...
        for pid in pids:
            ...
            if result.returncode == 0:
                killed = True
            # Kill parent process only if it is cargo.exe (which holds log file handles)
            if ppid:
...
def is_cargo_compiling() -> bool:
    ...
    Returns:
        True 如果有cargo/rustc进程正在编译，False otherwise
    """
    if os.name == "nt":
        # Windows: 使用PowerShell检测进程
        # 检测 cargo 和 rustc 进程
        command = (
