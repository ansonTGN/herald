...
def get_file_type(file_path: Path) -> str:
    ...
    if file_path.name in README_PATTERNS:
        return "readme"
    if relative_path.startswith(".claude/agents/"):
        return "agent"
    if relative_path.startswith(".claude/commands/"):
        return "command"
    if relative_path.startswith(".claude/skills/"):
        return "skill"
    if relative_path.startswith(".claude/protocols/"):
        return "protocol"
...
def is_entry_doc(file_path: Path) -> bool:
    ...
    if rel.startswith(".claude/commands/"):
        return True
    if rel.startswith(".claude/skills/") and file_path.name == "SKILL.md":
        return True
    return False
