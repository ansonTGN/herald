...
impl OAuthRepository for PostgresOAuthRepository {
    async fn find_by_provider_and_open_id(
        ...
        let (id, realm, provider, open_id, union_id, email, created_at, updated_at): OAuthProviderRow =
            sqlx::query_as(
                "SELECT id, realm_id, provider_type, open_id, union_id, email, created_at, updated_at
                 FROM provider
        ...
        Self::model_to_entity(
            id, realm, provider, open_id, union_id, email, created_at, updated_at,
        )
    }
    ...
    async fn find_by_user_id(&self, _user_id: Uuid) -> Result<Vec<OAuthProvider>, CoreError> {
        // This method requires linking provider to user via a junction table
        // For now, return empty vec until the linking is implemented
        Ok(vec![])
    }
    ...
    async fn create_provider(&self, provider: OAuthProvider) -> Result<OAuthProvider, CoreError> {
        let rec: OAuthProviderRow = sqlx::query_as(
                "INSERT INTO provider (id, realm_id, provider_type, open_id, union_id, email, created_at, updated_at)
                 VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
                 RETURNING id, realm_id, provider_type, open_id, union_id, email, created_at, updated_at",
            )
            .bind(provider.id)
        ...
        Self::model_to_entity(rec.0, rec.1, rec.2, rec.3, rec.4, rec.5, rec.6, rec.7)
    }
    ...
    async fn link_provider_to_user(
        ...
        // This requires a junction table between users and providers
        // For now, just return Ok until linking is implemented
        Ok(())
    }
