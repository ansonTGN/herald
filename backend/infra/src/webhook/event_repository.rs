...
mod tests {
    ...
    // in a separate test file to test the full webhook processing flow.

    #[test]
    fn test_idempotency_result_claimed() {
        let result = IdempotencyResult::Claimed {
            event_id: Uuid::now_v7(),
        };
        match result {
            IdempotencyResult::Claimed { event_id } => {
                assert!(event_id.get_version().is_some()); // Just check it's a valid UUID
            }
            _ => panic!("Expected Claimed variant"),
        }
    }

    #[test]
    fn test_idempotency_result_already_processed() {
        let result = IdempotencyResult::AlreadyProcessed {
            event_id: Uuid::now_v7(),
        };
        match result {
            ...
            _ => panic!("Expected AlreadyProcessed variant"),
        }
    }

    #[test]
    fn test_idempotency_result_in_progress() {
        let result = IdempotencyResult::InProgress {
            event_id: Uuid::now_v7(),
        };
        match result {
