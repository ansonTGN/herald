...
billing_repo_test!(test_repository_update_subscription_bucket_id, |repo| {
    // Regression: apply_subscription_update must set bucket_id on UPDATE,
    ...
    let reloaded = repo
        .find_subscription_by_id(created.id)
        .await
        .unwrap()
        .unwrap();
