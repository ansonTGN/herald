...
mod tests {
    ...
    fn test_webhook_context_creation() {
        let context = WebhookContext {
            ...
            external_event_id: "ext-123".to_string(),
            payment_provider: "shopify".to_string(),
            event_type: "subscription_contracts/create".to_string(),
            payload: serde_json::json!({"test": "data"}),
        };

        assert_eq!(context.realm_id, "test-realm");
        assert_eq!(context.payment_provider, "shopify");
        assert_eq!(context.event_type, "subscription_contracts/create");
    }

    #[test]
    fn test_webhook_process_result_processed() {
        let result = WebhookProcessResult::Processed {
            event_id: Uuid::now_v7(),
        };
        match result {
            WebhookProcessResult::Processed { event_id } => {
                assert!(event_id.get_version().is_some()); // Just verify it's a valid UUID
            }
    ...
    #[test]
    fn test_webhook_process_result_skipped() {
        let result = WebhookProcessResult::Skipped {
            event_id: Uuid::now_v7(),
        };
        match result {
    ...
    #[test]
    fn test_webhook_process_result_in_progress() {
        let result = WebhookProcessResult::InProgress {
            event_id: Uuid::now_v7(),
        };
        match result {
