...
pub async fn create_third_party_test_subscription(
    ...
    let plan_id_uuid = if let Some(name) = plan_name {
        ...
        sqlx::query(
            "INSERT INTO subscription_plan (id, realm_id, name, description, title, type, price, currency, active, trial_days, sort_order, product_id, created_at, updated_at)
    ...
    sqlx::query(
        r#"
        INSERT INTO subscription (id, realm_id, external_subscription_id, external_product_id, payment_provider, client_app_id, plan_id, status, tier, billing_period, current_period_start, current_period_end, created_at)
        VALUES ($1, $2, $3, $4, 'creem', $5, $6, $7, $8, $9, $10, $11, $12)
        "#,
    )
    .bind(subscription_id)
    .bind(&ctx._realm_id)
    .bind(format!("ext-sub-thirdparty-{}", subscription_id))
    .bind("test_product_dummy") // Dummy external_product_id (required by schema)
    .bind(*client_app_id)
    .bind(plan_id_uuid)
    .bind(status)
    .bind(tier)
...
/// TODO: Consolidate with test-support::helpers::client_helpers::grant_api_key_permissions
