...
/// # Example
/// ```rust
/// let response = make_authenticated_get_request(ctx, &token, "/users", Some("page=2&pageSize=10")).await;
/// assert_response_pagination(&response, 2, 10, 100);
/// ```
pub fn assert_response_pagination(
    ...
    assert_eq!(
        response.get("page").and_then(|p| p.as_i64()),
        Some(expected_page),
        "Page should be {}",
        expected_page
    );
    assert_eq!(
        response.get("pageSize").and_then(|ps| ps.as_i64()),
        Some(expected_page_size),
        "Page size should be {}",
        expected_page_size
    );
    assert_eq!(
        response.get("total").and_then(|t| t.as_i64()),
        Some(expected_total),
        "Total should be {}",
        expected_total
    );
