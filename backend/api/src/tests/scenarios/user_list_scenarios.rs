...
///
/// **验收标准**：
/// - page=0, pageSize=5 返回前 5 条记录
/// - page=1, pageSize=5 返回第 6-10 条记录
/// - pagination.total_pages 计算正确
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_admin_list_users_pagination
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_admin_list_users_pagination(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    // ============================================================================
    let (admin_token, admin_user_id) =
        create_admin_session_with_user(ctx, "admin-pagination@test.com", 1800).await;
    grant_realm_admin_role(ctx, &admin_user_id).await;

    // 创建一些测试用户
    println!("[Setup] 创建测试用户");
    for i in 0..15 {
        let email = format!("pagination_user_{}@test.com", i);
        create_simple_test_user(ctx, &email).await;
    }
    println!("[Setup] ✓ 创建了 15 个测试用户");

    // ============================================================================
    // Test 1: 第一页 (page=0, pageSize=5)
    // ============================================================================
    println!("\n[Test 1] 请求第一页: page=0, pageSize=5");

    let req = Request::builder()
        .method("GET")
        .uri(format!(
            "/api/roles/{}/users?page=0&pageSize=5",
            ctx._realm_id
        ))
        .header(header::COOKIE, format!("X-Auth={}", admin_token))
        .body(Body::empty())
    ...
    let resp = app.clone().oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK, "第一页请求应该成功");

    let body: serde_json::Value = crate::tests::response_json(resp).await;
    let users = body["items"].as_array().unwrap();

    assert_eq!(users.len(), 5, "第一页应该返回 5 个用户");
    assert_eq!(body["page"], 0, "page 应该是 0");
    assert_eq!(body["pageSize"], 5, "page_size 应该是 5");
    assert!(
        body["total"].as_i64().unwrap_or(0) >= 15,
        "total 应该至少包含创建的测试用户"
    );

    println!("[Test 1] ✅ 第一页返回正确: {} 个用户", users.len());

    // ============================================================================
    // Test 2: 第二页 (page=1, pageSize=5)
    // ============================================================================
    println!("\n[Test 2] 请求第二页: page=1, pageSize=5");

    let req = Request::builder()
        .method("GET")
        .uri(format!(
            "/api/roles/{}/users?page=1&pageSize=5",
            ctx._realm_id
        ))
        .header(header::COOKIE, format!("X-Auth={}", admin_token))
        .body(Body::empty())
    ...
    let resp = app.clone().oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK, "第二页请求应该成功");

    let body: serde_json::Value = crate::tests::response_json(resp).await;
    let users = body["items"].as_array().unwrap();

    assert_eq!(users.len(), 5, "第二页应该返回 5 个用户");
    assert_eq!(body["page"], 1, "page 应该是 1");
    assert_eq!(body["pageSize"], 5, "page_size 应该是 5");

    println!("[Test 2] ✅ 第二页返回正确: {} 个用户", users.len());

    // ============================================================================
    // Test 3: 最后一页 (可能不足 5 条)
    // ============================================================================
    println!("\n[Test 3] 请求第三页: page=2, pageSize=5");

    let req = Request::builder()
        .method("GET")
        .uri(format!(
            "/api/roles/{}/users?page=2&pageSize=5",
            ctx._realm_id
        ))
        .header(header::COOKIE, format!("X-Auth={}", admin_token))
        .body(Body::empty())
    ...
    let resp = app.clone().oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK, "第三页请求应该成功");

    let body: serde_json::Value = crate::tests::response_json(resp).await;
    let users = body["items"].as_array().unwrap();

    println!(
        "[Test 3] ✅ 第三页返回: {} 个用户 (可能是最后一页)",
        users.len()
    );
...
///
/// **场景描述**：
/// 当不提供分页参数时，API 应该使用默认值。
///
/// **测试步骤**：
/// 1. 调用 API 时不提供 page 和 pageSize 参数
/// 2. 验证使用默认值 (page=0, pageSize=10)
///
/// **验收标准**：
/// - 默认返回 page=0
/// - 默认返回 pageSize=10
...
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_admin_list_users_default_pagination
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_admin_list_users_default_pagination(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    // ============================================================================
    let (admin_token, admin_user_id) =
        create_admin_session_with_user(ctx, "admin-default@test.com", 1800).await;
    grant_realm_admin_role(ctx, &admin_user_id).await;

    // ============================================================================
    // Test: 不提供分页参数
    // ============================================================================
    println!("[Test] 请求用户列表（不提供分页参数）");

    let req = Request::builder()
        .method("GET")
        .uri(format!("/api/roles/{}/users", ctx._realm_id))
        .header(header::COOKIE, format!("X-Auth={}", admin_token))
        .body(Body::empty())
        .unwrap();

    let resp = app.clone().oneshot(req).await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK, "请求应该成功");

    let body: serde_json::Value = crate::tests::response_json(resp).await;
    assert_eq!(body["page"], 0, "默认 page 应该是 0");
    assert_eq!(body["pageSize"], 10, "默认 pageSize 应该是 10");

    println!("[Test] ✅ 默认分页参数正确: page=0, pageSize=10");
