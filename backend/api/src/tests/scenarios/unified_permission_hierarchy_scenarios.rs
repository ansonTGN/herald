...
mod tests {
    ...
    /// 场景测试：Manage 权限包含 View 权限（Direct）
    ///
    /// **Given**: 用户拥有 `users:manage` 权限
    ...
    #[test_context(PermissionHierarchyTestContext)]
    #[tokio::test]
    async fn test_scenario_manage_permission_includes_view_direct(
        ctx: &mut PermissionHierarchyTestContext,
    ) {
        execute_permission_hierarchy_test(
            ctx,
    ...
    /// 场景测试：View 权限不包含 Manage 权限（Direct）
    ///
    /// **Given**: 用户拥有 `users:view` 权限
    /// **When**: 检查用户是否有 `users:manage` 权限
    /// **Then**: 权限检查返回 false（拒绝访问）
    #[test_context(PermissionHierarchyTestContext)]
    #[tokio::test]
    async fn test_scenario_view_permission_does_not_include_manage_direct(
        ctx: &mut PermissionHierarchyTestContext,
    ) {
        execute_permission_hierarchy_test(
            ctx,
    ...
    #[test_context(PermissionHierarchyTestContext)]
    #[tokio::test]
    async fn test_scenario_custom_permissions_require_exact_match_direct(
        ctx: &mut PermissionHierarchyTestContext,
    ) {
        execute_permission_hierarchy_test(
            ctx,
    ...
    ///
    /// **Given**: 用户拥有 `users:manage` 和 `realm:view` 权限
    /// **When**: 检查用户是否有 `users:view` 和 `realm:view` 权限
    /// **Then**: 两个权限检查都返回 true（允许访问）
    #[test_context(PermissionHierarchyTestContext)]
    #[tokio::test]
    async fn test_scenario_multiple_permissions_with_hierarchy_direct(
        ctx: &mut PermissionHierarchyTestContext,
    ) {
        execute_permission_hierarchy_test(
            ctx,
