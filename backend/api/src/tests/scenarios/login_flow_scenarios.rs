...
///
/// **场景描述**：
/// 使用普通用户账号（realm1）登录系统，验证登录 API 返回正确的响应。
///
/// **测试步骤**：
/// 1. 在 realm1 中创建测试用户（status=1，已激活状态）
/// 2. 使用邮箱和密码登录
/// 3. 验证响应状态码为 200
/// 4. 验证返回的 JSON 包含正确的 user_id
/// 5. 验证 Set-Cookie 包含 X-Auth token
/// 6. 验证 session 存储在 Redis 中
///
/// **验收标准**：
/// - 登录成功，返回 200 状态码
/// - 响应包含 user_id 和 message="ok"
/// - Set-Cookie 包含 X-Auth token
/// - Session 存储在 Redis 中，包含正确的 user_id, realm_id, client_id
///
/// **测试账号**：
/// - Email: newuser@cas.com
/// - Password: password123
/// - Realm: realm1
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_user_login_success
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_user_login_success(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    // ============================================================================
    // Step 2-3: 使用邮箱和密码登录，验证响应状态码为 200
    // ============================================================================
    println!("[Step 2-3] 验证登录响应");
    let payload = json!({
        "clientId": ctx._client_id,
        "email": email,
        "password": password,
    ...
    println!("[Step 2-3] ✓ 登录请求完成，状态码为 200");

    // ============================================================================
    // Step 4: 验证返回的 JSON 包含正确的 user_id
    // ============================================================================
    ...
    // requires_totp can be null or false for users without TOTP
    let requires_totp = &body["requiresTotp"];
    assert!(
        requires_totp.is_null() || requires_totp.as_bool() == Some(false),
        "requires_totp should be null or false for user without TOTP, got {:?}",
        requires_totp
    );
    assert_eq!(
        body["expiresInSeconds"], 1800,
        "Session should expire in 1800 seconds (30 minutes)"
    );
    println!("[Step 4] ✓ 响应 JSON 正确: user_id={}", body["userId"]);

    // ============================================================================
    // Step 5: 验证 Set-Cookie 包含 X-Auth token
    // ============================================================================
    println!("[Step 5] 验证 Set-Cookie 包含 X-Auth token");
    // 由于前面已经消费了 response，我们需要再次登录来获取 headers
    let request = Request::builder()
        .method("POST")
        .uri(format!("/api/auth/{}/login", ctx._realm_id))
        .header("content-type", "application/json")
    ...
    println!("[Step 5] ✓ Set-Cookie 包含 X-Auth token: {}", token);
    ...
    assert_eq!(
        session.realm_id, ctx._realm_id,
        "Session realm_id should match"
    );
    assert_eq!(
        session.client_id, ctx._client_id,
        "Session client_id should match"
...
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_user_login_nonexistent_user
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_user_login_nonexistent_user(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();

    // ============================================================================
    // Step 1: 使用不存在的邮箱登录
    // ============================================================================
    println!("[Step 1] 使用不存在的邮箱登录");
    let payload = json!({
        "clientId": ctx._client_id,
        "email": "nonexistent@cas.com",
        "password": "password123",
        "turnstileToken": "dummy"
    });
    ...
    // ============================================================================
    // Step 2: 验证返回 401 Unauthorized
    // ============================================================================
    println!("[Step 2] 验证返回 401 Unauthorized");
    assert_eq!(
        response.status(),
        StatusCode::UNAUTHORIZED,
    ...
    println!("[Step 2] ✓ 返回 401 Unauthorized，错误信息正确");

    println!("\n✅ User Story 完成：不存在用户登录被正确拒绝");
}
