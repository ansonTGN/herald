...
mod tests {
    ...
    /// User Story: US-PP-011 验收标准 4
    /// 场景：相同 event_id 已在处理中时，重复投递立即返回 202 且不重复执行业务副作用
    #[test_context(ShopifyTestContext)]
    #[tokio::test]
    async fn test_scenario_shopify_subscription_contracts_create_in_progress_duplicate_is_accepted_without_side_effects(
        ctx: &mut ShopifyTestContext,
    ) {
        let app = ctx.create_unified_test_router();
        let realm_id = ctx._realm_id.clone();
        ...
        sqlx::query(
            "INSERT INTO payment_event
                (id, realm_id, external_event_id, payment_provider, event_type, payload, processed, processing_started_at, created_at)
             VALUES
                ($1, $2, $3, 'shopify', $4, $5, false, NOW(), NOW())",
        )
        .bind(Uuid::now_v7())
        .bind(&realm_id)
        .bind(&event_id)
        .bind("subscription_contracts/create")
        .bind(&payload)
        .execute(&ctx.app_state.pool)
        .await
        .expect("Failed to seed in-progress payment event");
        ...
        assert_eq!(
            ...
            "In-progress duplicate should not create a second subscription side effect"
        );

        let event = get_payment_event(ctx, &event_id)
            .await
            .expect("Payment event should still exist");
        assert_eq!(event["processed"], false);
        assert!(
            !event["processingStartedAt"].is_null(),
            "In-progress event should keep its processing lease"
        );
        ...
        assert_eq!(
            user_points, 0,
            "In-progress duplicate should not grant points"
        );
    }
    ...
    /// User Story: US-PP-012
    /// 场景：v1 简化模型下，退款按用户撤销全部未使用订阅积分
    #[test_context(ShopifyTestContext)]
    #[tokio::test]
    async fn test_scenario_shopify_refunds_revoke_all_user_subscription_points_v1(
        ctx: &mut ShopifyTestContext,
    ) {
        let app = ctx.create_unified_test_router();
        let realm_id = ctx._realm_id.clone();
        ...
        let contract_id_1 = generate_shopify_contract_id();
        let contract_id_2 = generate_shopify_contract_id();
        let customer_id_1 = generate_shopify_customer_id();
        let customer_id_2 = generate_shopify_customer_id();
        let order_id_1 = generate_shopify_order_id();

        let payload_1 = build_shopify_subscription_contracts_create_event(
            generate_test_event_id(),
            contract_id_1.clone(),
            customer_id_1,
            user_id,
        ...
        let payload_2 = build_shopify_subscription_contracts_create_event(
            generate_test_event_id(),
            contract_id_2.clone(),
            customer_id_2,
            user_id,
        ...
        let response_1 = send_shopify_webhook_with_signature(
            &app,
            &realm_id,
            "subscription_contracts/create",
            payload_1,
        ...
        .await;
        let response_2 = send_shopify_webhook_with_signature(
            &app,
            &realm_id,
            "subscription_contracts/create",
            payload_2,
        ...
        assert_shopify_webhook_success(&response_1);
        assert_shopify_webhook_success(&response_2);
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
        ...
        .execute(&ctx.app_state.pool)
        .await
        .expect("Failed to attach order to first subscription");

        let refund_payload = build_shopify_refunds_create_event(
            generate_test_event_id(),
            format!("gid://shopify/Refund/{}", Uuid::now_v7()),
            order_id_1,
            2500,
            "USD",
            Some("Customer request"),
        ...
        let refund_response = send_shopify_webhook_with_signature(
            &app,
            &realm_id,
            "refunds/create",
            refund_payload,
        ...
        assert_shopify_webhook_success(&refund_response);
        tokio::time::sleep(tokio::time::Duration::from_millis(100)).await;
        ...
        assert_eq!(
            ...
            "v1 Shopify refund handling should revoke all unused subscription credits for the user"
        );

        let remaining_subscription_points: i64 = sqlx::query_scalar(
            "SELECT CAST(COALESCE(SUM(remaining_amount), 0) AS BIGINT) FROM points_credit_ledger
        ...
        assert_eq!(
            remaining_subscription_points, 0,
            "All user subscription ledgers should be exhausted after a Shopify refund in v1"
        );
    }
}
