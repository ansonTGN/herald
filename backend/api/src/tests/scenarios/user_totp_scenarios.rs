...
/// ============================================================================
/// User Story: TOTP Login - Invalid Code Failure
///
/// **场景描述**：
/// TOTP 验证码错误（失败场景）。
///
...
/// 1. 创建用户并启用 TOTP
/// 2. 调用登录 API 获取 temp_token
/// 3. 使用无效代码 `000000` 调用 verify-totp
/// 4. 验证错误响应
/// 5. 确认 temp_token 被删除（安全措施）
///
/// **验收标准**：
/// - 返回 401 状态码
/// - 错误消息正确（"验证码已过期或无效"）
...
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_totp_login_invalid_code_failure
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_totp_login_invalid_code_failure(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    // ============================================================================
    println!("[Setup] 创建用户并启用 TOTP");
    let email = "totp_login_user2@cas.com";
    let password = "password123";
    create_test_user(ctx, email, password).await;
    setup_realm_totp_config(ctx, true, false).await;
    ...
    // ============================================================================
    // Step 2: 使用无效代码调用 verify-totp
    // ============================================================================
    println!("[Step 2] 使用无效代码调用 verify-totp");
    let realm_id = ctx._realm_id.clone();
    let result = complete_totp_login(ctx, &realm_id, &temp_token, Some("000000"), None).await;
    ...
    // ============================================================================
    // Step 3: 验证 temp_token 已删除
    // ============================================================================
    println!("[Step 3] 验证 temp_token 已删除");
    ...
    println!("\n✅ User Story 完成：TOTP Login - Invalid Code Failure");
}
...
/// ============================================================================
/// User Story: TOTP Login Not Enabled - Direct Login
///
/// **场景描述**：
/// 未启用 TOTP 的用户直接登录。
///
...
/// 1. 创建用户（不启用 TOTP）
/// 2. Realm 配置 TOTP（非强制）
/// 3. 调用登录 API（直接返回 session_token，不需要 verify-totp）
/// 4. 验证登录成功
///
/// **验收标准**：
/// - 返回 200 状态码
/// - 直接返回 session_token（不返回 temp_token）
/// - 无需 TOTP 验证步骤
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_totp_login_not_enabled_direct_login
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_totp_login_not_enabled_direct_login(ctx: &mut TestContext) {
    unsafe {
        std::env::set_var("TOTP_SECRET_KEY", "test_key_32_bytes_long_1234567890");
    }
    ...
    // Setup: 创建用户（不启用 TOTP）
    // ============================================================================
    println!("[Setup] 创建用户（不启用 TOTP）");
    let email = "totp_login_user6@cas.com";
    let password = "password123";
    create_test_user(ctx, email, password).await;
    setup_realm_totp_config(ctx, true, false).await; // Realm TOTP enabled, but not forced
    println!("[Setup] ✓ 用户创建成功，未启用 TOTP");

    // ============================================================================
    // Step 1: 调用登录 API
    // ============================================================================
    println!("[Step 1] 调用登录 API");
    let (token, _) = create_temp_totp_session(ctx, email, password).await;
    println!("[Step 1] ✓ 登录成功, token: {}", token);

    // ============================================================================
    // Step 2: 验证返回的是 session_token（不包含 temp_token 前缀）
    // ============================================================================
    println!("[Step 2] 验证返回的是 session_token");
    assert!(
        !token.starts_with("totp_login_"),
        "Should not return temp_token"
    );
    println!("[Step 2] ✓ 返回的是 session_token");

    // ============================================================================
    // Step 3: 验证登录成功（可以使用 session_token 访问受保护资源）
    // ============================================================================
    println!("[Step 3] 验证登录成功");
    // Session token format: {realm_id}_{uuid}_{timestamp}
    assert!(
        token.contains(&ctx._realm_id),
        "Session token should contain realm_id"
    );
    // Verify token has 3 parts (realm_id, uuid, timestamp)
    let parts: Vec<&str> = token.split('_').collect();
    assert_eq!(parts.len(), 3, "Session token should have 3 parts");
    println!("[Step 3] ✓ 登录成功验证通过");
    ...
    println!("\n✅ User Story 完成：TOTP Login Not Enabled - Direct Login");
}
...
/// ============================================================================
/// User Story: TOTP Enable - Realm Not Enabled Failure
///
/// **场景描述**：
/// Realm 未启用 TOTP。
///
/// **测试步骤**：
/// 1. 创建测试用户
/// 2. 不配置 Realm TOTP（enabled=false）
/// 3. 登录
/// 4. 尝试启用 TOTP
/// 5. 验证错误响应
///
/// **验收标准**：
/// - 返回 400 状态码
/// - 错误消息："Realm TOTP is not enabled"
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_totp_enable_realm_not_enabled_failure
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_totp_enable_realm_not_enabled_failure(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    println!("[Setup] 创建测试用户");
    let email = "totp_enable_user2@cas.com";
    let password = "password123";
    create_test_user(ctx, email, password).await;
    // Realm TOTP not configured
    setup_realm_totp_config(ctx, false, false).await;
    ...
    println!("[Step 1] 尝试启用 TOTP（Realm 未启用）");
    let enable_payload = json!({ "password": password });
    let enable_request = Request::builder()
        .method("POST")
        .uri("/api/user/totp")
    ...
    assert_eq!(
        enable_body["message"], "Realm TOTP is not enabled",
        "Should return correct error message"
    );
    println!("[Step 1] ✓ 错误消息正确");
    ...
    println!("\n✅ User Story 完成：TOTP Enable - Realm Not Enabled Failure");
}
...
/// ============================================================================
/// User Story: TOTP Enable - Duplicate Failure
///
/// **场景描述**：
/// 重复启用 TOTP（失败场景）。
///
...
/// **验收标准**：
/// - 返回 409 状态码
/// - 错误消息："TOTP already enabled"
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_totp_enable_duplicate_failure
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_totp_enable_duplicate_failure(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    let email = "totp_enable_user4@cas.com";
    let password = "password123";
    create_test_user(ctx, email, password).await;
    setup_realm_totp_config(ctx, true, false).await;
    ...
    let enable_response2 = app.clone().oneshot(enable_request2).await.unwrap();
    assert_eq!(enable_response2.status(), StatusCode::CONFLICT);
    println!("[Step 1] ✓ 返回 409 状态码");

    let enable_body_bytes2 = axum::body::to_bytes(enable_response2.into_body(), usize::MAX)
        .await
        .expect("Failed to read response body");
    let enable_body2: serde_json::Value =
        serde_json::from_slice(&enable_body_bytes2).expect("Failed to parse JSON");

    let actual_message = enable_body2["message"]
        .as_str()
        .expect("Error message should be a string");
    assert_eq!(
        actual_message, "TOTP already enabled",
        "Should return correct error message"
    );
    println!("[Step 1] ✓ 错误消息正确");
    ...
    println!("\n✅ User Story 完成：TOTP Enable - Duplicate Failure");
}

/// ============================================================================
/// User Story: TOTP Enable - Code Expired Failure
///
/// **场景描述**：
/// 验证码过期（超时测试）。
///
...
/// 3. 登录
/// 4. 启动 TOTP setup
/// 5. 等待 temp_token 过期（5 分钟）
/// 6. 尝试验证
/// 7. 验证错误响应
///
/// **验收标准**：
/// - 返回 401 状态码
/// - 错误消息："Invalid or expired temporary token"
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_totp_enable_code_expired_failure
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_totp_enable_code_expired_failure(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    println!("[Setup] 创建测试用户");
    let email = "totp_enable_user5@cas.com";
    let password = "password123";
    create_test_user(ctx, email, password).await;
    setup_realm_totp_config(ctx, true, false).await;
    ...
    let temp_token = enable_body["tempToken"].as_str().unwrap();
    println!("[Step 1] ✓ TOTP setup 启动成功, temp_token: {}", temp_token);

    println!("[Step 2] 手动删除 temp_token 模拟过期");
    ...
    let _: () = redis::cmd("DEL")
        ...
        .await
        .unwrap();
    println!("[Step 2] ✓ temp_token 已删除");

    println!("[Step 3] 尝试验证（temp_token 已过期）");
    let totp_code = "123456"; // Any code will fail
    let verify_payload = json!({
        "tempToken": temp_token,
        "code": totp_code
    });
    ...
    assert_eq!(
        verify_body["message"], "Invalid or expired temporary token",
        "Should return correct error message"
    );
    println!("[Step 3] ✓ 错误消息正确");
    ...
    println!("\n✅ User Story 完成：TOTP Enable - Code Expired Failure");
}

// ============================================================================
// Phase 3: US-TO-004 Disable Scenarios (2 tests)
// ============================================================================

/// ============================================================================
/// User Story: TOTP Disable - Invalid Password Failure
///
/// **场景描述**：
/// 密码验证失败（失败场景）。
///
...
/// 5. 使用错误密码禁用 TOTP
/// 6. 验证错误响应
/// 7. 检查 TOTP 配置仍为 enabled=true
///
/// **验收标准**：
/// - 返回 401 状态码
/// - TOTP 配置仍为 enabled=true
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_totp_disable_invalid_password_failure
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_totp_disable_invalid_password_failure(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    println!("[Setup] 创建测试用户并启用 TOTP");
    let email = "totp_disable_user1@cas.com";
    let password = "password123";
    create_test_user(ctx, email, password).await;
    setup_realm_totp_config(ctx, true, false).await;
    ...
    println!("[Step 1] 使用错误密码禁用 TOTP");
    let disable_payload = json!({ "password": "wrongpassword" });
    let disable_request = Request::builder()
        .method("DELETE")
        .uri("/api/user/totp")
        .header("content-type", "application/json")
    ...
    println!("[Step 2] 检查 TOTP 配置仍为 enabled=true");
    let user_id: String = sqlx::query_scalar("SELECT id::text FROM account WHERE email = $1")
        .bind(email)
        .fetch_one(&ctx._app_state.pool)
        .await
    ...
    let config_enabled: Option<bool> =
        sqlx::query_scalar("SELECT enabled FROM user_totp_config WHERE user_id = $1::uuid")
            ...
            .await
            .unwrap();
    assert_eq!(config_enabled, Some(true), "TOTP should still be enabled");
    println!("[Step 2] ✓ TOTP 配置仍为 enabled=true");
    ...
    println!("\n✅ User Story 完成：TOTP Disable - Invalid Password Failure");
}

/// ============================================================================
/// User Story: TOTP Disable - Realm Force Enabled Failure
///
/// **场景描述**：
/// Realm 强制启用 TOTP（失败场景）。
///
/// **测试步骤**：
/// 1. 创建测试用户
/// 2. 配置 Realm TOTP（force_enabled=true）
/// 3. 登录
/// 4. 启用并验证 TOTP
/// 5. 尝试禁用 TOTP
/// 6. 验证错误响应
...
/// **验收标准**：
/// - 返回 403 状态码
/// - 错误消息："TOTP is enforced by realm"
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_totp_disable_realm_force_enabled_failure
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_totp_disable_realm_force_enabled_failure(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    println!("[Setup] 创建测试用户并启用 TOTP");
    let email = "totp_disable_user2@cas.com";
    let password = "password123";
    create_test_user(ctx, email, password).await;
    setup_realm_totp_config(ctx, true, true).await; // Force enabled
    ...
    println!("[Step 1] 尝试禁用 TOTP（Realm 强制启用）");
    let disable_payload = json!({ "password": password });
    let disable_request = Request::builder()
        .method("DELETE")
        .uri("/api/user/totp")
    ...
    assert_eq!(
        disable_body["message"], "TOTP is enforced by realm",
        "Should return correct error message"
    );
    println!("[Step 1] ✓ 错误消息正确");
    ...
    println!("\n✅ User Story 完成：TOTP Disable - Realm Force Enabled Failure");
}
