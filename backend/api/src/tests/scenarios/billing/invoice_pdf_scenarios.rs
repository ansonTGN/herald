...
mod tests {
    ...
    // User Story: docs/user-stories/13-invoice-user-stories.md
    // Covers: US-IV-009 -- admin download paid invoice returns application/pdf
    //
    // Given: An admin with billing.view permission and a paid invoice
    // When: GET /api/bill/{realmId}/invoices/{invoiceId}/pdf
    // Then: Returns 200 with Content-Type: application/pdf

    #[test_context(InvoiceTestContext)]
    #[tokio::test]
    async fn test_admin_download_paid_invoice_pdf(ctx: &mut InvoiceTestContext) {
        let app = ctx.create_unified_test_router();
        let realm_id = ctx._realm_id.clone();
        let admin_token = setup_billing_admin_session(ctx, "invoice-pdf-admin-paid@test.com").await;
        let account_id = ensure_test_account(ctx, &realm_id).await;
        ...
        // Download the PDF for a paid invoice
        let pdf_response = app
            .clone()
            .oneshot(
                Request::builder()
