...
mod tests {
    ...
    /// ============================================================================
    /// Scenario 3: Subscription fulfillment field mapping
    /// ============================================================================
    /// User Story: docs/user-stories/12-payment-attempt-user-stories.md
    /// Covers: US-PA-003 - 验收标准：订阅字段正确填充
    ///
    /// Scenario: All subscription fields are correctly mapped from PaymentAttempt
    /// Given: A payment attempt with complete field information
    /// When: Fulfillment service processes the subscription purchase
    /// Then: All subscription fields match the expected mapping from PaymentAttempt

    #[test_context(TestContext)]
    #[tokio::test]
    async fn test_scenario_subscription_fulfillment_field_mapping(ctx: &mut TestContext) {
        let realm_id = ctx._realm_id.clone();
        ...
        let provider_transaction_id = format!("stripe_premium_{}", payment_attempt_id);
        ...
        // Verify field mapping table
        assert_eq!(sub_realm_id, realm_id, "realm_id should match");
        assert_eq!(sub_user_id, Some(user_id), "user_id should match");
        assert_eq!(sub_plan_id, Some(plan_id), "plan_id should match target_id");
        assert_eq!(
            sub_payment_provider, "stripe",
            "payment_provider should match"
        );
        assert_eq!(sub_status_str, "active", "status should be Active");
        assert_eq!(
            sub_tier_str, "professional",
            "tier should be Professional (P0 default)"
        );

        // Verify period fields
        let period_data = get_subscription_period(ctx, &provider_transaction_id).await;
        assert!(period_data.is_some(), "Period data should exist");
        let (period_start, _period_end) = period_data.unwrap();
        assert!(
            period_start <= chrono::Utc::now() + chrono::Duration::minutes(5),
            "period_start should be recent"
        );

        // Note: We can't easily verify external_subscription_id and external_product_id without query_as
        // The key fields we can verify are: realm_id, user_id, plan_id, payment_provider, status, tier
    }
    ...
    /// ============================================================================
    /// Scenario 9: Subscription lifecycle - Active to Canceled
    /// ============================================================================
    /// User Story: docs/user-stories/12-payment-attempt-user-stories.md
    /// Covers: US-PA-003 - 验收标准：正确处理订阅状态
    ///
    /// Scenario: Subscription can be canceled after fulfillment
    /// Given: An active subscription
    /// When: Subscription is canceled
    /// Then: Subscription status changes to Canceled
    /// And: cancel_at field is set

    #[test_context(TestContext)]
    #[tokio::test]
    async fn test_scenario_subscription_lifecycle_cancellation(ctx: &mut TestContext) {
        let realm_id = ctx._realm_id.clone();

        // Given: Create and fulfill subscription
        let plan_id = create_test_plan(ctx, &realm_id, "Cancelable Plan").await;
        let user_id = Uuid::now_v7();
        let payment_attempt_id = create_payment_attempt(
            ctx,
            &realm_id,
        ...
        let provider_transaction_id = format!("stripe_cancel_{}", payment_attempt_id);
        ...
        // Get subscription ID
        let subscription_id = get_subscription_by_external_id(ctx, &provider_transaction_id)
            .await
            .expect("Subscription should exist")
            .0;
        ...
        // Then: Verify subscription status by checking status column
