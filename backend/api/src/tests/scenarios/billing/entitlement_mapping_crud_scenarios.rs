...
mod tests {
    ...
    /// User Story: US-EM-004
    /// Covers: CHECK constraint chk_pem_entitlement_key rejects invalid keys
    #[test_context(EntitlementTestContext)]
    #[tokio::test]
    async fn test_entitlement_key_db_check_constraint_rejects_invalid(
        ctx: &mut EntitlementTestContext,
    ) {
        let realm_id = ctx._realm_id.clone();
        ...
        assert!(
            result.is_err(),
            "Should reject special characters in entitlement_key"
        );
        ...
        .execute(pool)
        .await;
        assert!(result.is_err(), "Should reject >64 char entitlement_key");
    ...
    /// User Story: US-EM-001, US-EM-004
    /// Covers: provider_entitlement_mappings exists with correct columns; old tables removed;
    ///         subscription table has new fields; points_grant_schedules has entitlement_key
    #[test_context(EntitlementTestContext)]
    #[tokio::test]
    async fn test_migration_schema_verification(ctx: &mut EntitlementTestContext) {
        let pool = &ctx.app_state.pool;

        // 1. Verify provider_entitlement_mappings table exists with correct columns
        let pem_columns: Vec<String> = sqlx::query_scalar(
        ...
        for col in &required_columns {
            assert!(
                pem_columns.iter().any(|c| c == col),
                "Missing column '{}' in provider_entitlement_mappings",
                col
            );
        }

        // 2. Verify old tables do NOT exist
        let old_tables = ["products", "subscription_plan", "points_plan_configs"];
        for table in &old_tables {
            ...
            .await
            .unwrap();
            assert_eq!(count, 0, "Old table '{}' should not exist", table);
        }

        // 3. Verify subscription table has new fields and lacks old fields
        let sub_columns: Vec<String> = sqlx::query_scalar(
            "SELECT column_name FROM information_schema.columns
        ...
        for col in &[
            ...
            assert!(
                sub_columns.iter().any(|c| c == col),
                "Missing new column '{}' in subscription",
                col
            );
        }

        // Old fields must NOT exist
        for col in &["plan_id", "tier", "billing_period"] {
            assert!(
                !sub_columns.iter().any(|c| c == col),
                "Old column '{}' should not exist in subscription",
                col
            );
        }

        // 4. Verify points_grant_schedules has entitlement_key and lacks plan_config_id
        let pgs_columns: Vec<String> = sqlx::query_scalar(
            "SELECT column_name FROM information_schema.columns
             WHERE table_name = 'points_grant_schedules'
             ORDER BY ordinal_position",
        )
        .fetch_all(pool)
        .await
        ...
        assert!(
            pgs_columns.iter().any(|c| c == "entitlement_key"),
            "points_grant_schedules should have entitlement_key"
        );
        assert!(
            !pgs_columns.iter().any(|c| c == "plan_config_id"),
            "points_grant_schedules should not have plan_config_id"
        );

        // 5. Verify SeaORM entity can read from the new table (basic query)
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM provider_entitlement_mappings WHERE realm_id = 'nonexistent'",
        )
        .fetch_one(pool)
        .await
        .unwrap();
        assert_eq!(
            count, 0,
            "Should be able to query provider_entitlement_mappings"
        );
    }
}
