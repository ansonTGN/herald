...
mod tests {
    ...
    #[tokio::test]
    async fn test_scenario_webhook_signature_verification_success(ctx: &mut WebhookTestContext) {
    ...
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_webhook_signature_verification_failure(ctx: &mut WebhookTestContext) {
        let app = ctx.create_unified_test_router();
        let webhook_secret = "test_webhook_secret";
        let event_id = format!("evt_test_{}", Uuid::now_v7());
        ...
        // Step 2: 使用错误的签名（纯十六进制格式）
        // 使用有效的 hex 格式，但是错误的签名值
        let signature_header =
            "aabbccddeeff00112233445566778899aabbccddeeff00112233445566778899aabb".to_string();
        ...
        // Step 4: 验证返回 401 Unauthorized（或 400 Bad Request，取决于实现）
    ...
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_webhook_missing_signature_header(ctx: &mut WebhookTestContext) {
        let app = ctx.create_unified_test_router();
        let webhook_secret = "test_webhook_secret";
        let event_id = format!("evt_test_{}", Uuid::now_v7());
        ...
        // Step 3: 验证返回 401 Unauthorized（或 400 Bad Request）
        let status = response.status();
        assert!(
            status == StatusCode::UNAUTHORIZED || status == StatusCode::BAD_REQUEST,
            "Expected 401 or 400 for missing signature, got {}",
            status
        );
    }
    ...
    /// Test: Webhook 签名头格式错误
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_webhook_invalid_signature_format(ctx: &mut WebhookTestContext) {
        let app = ctx.create_unified_test_router();
        let webhook_secret = "test_webhook_secret";
        let event_id = format!("evt_test_{}", Uuid::now_v7());
        ...
        // Step 2: 使用格式错误的签名头
        let signature_header = "invalid_format_signature".to_string();
        ...
        // Step 4: 验证返回 401 Unauthorized（或 400 Bad Request）
        let status = response.status();
        assert!(
            status == StatusCode::UNAUTHORIZED || status == StatusCode::BAD_REQUEST,
            "Expected 401 or 400 for invalid signature format, got {}",
            status
        );
    }
    ...
    // User Story: docs/user-stories/07-payment-provider-user-stories.md
    // Covers: US-PP-001 (配置支付平台), US-PP-002 (查看支付平台配置)

    /// Test: Stripe configuration can be stored and retrieved
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_stripe_configuration_storage(ctx: &mut WebhookTestContext) {
        ...
        let stored_api_key: Option<String> = sqlx::query_scalar(
            "SELECT config_value FROM realm_config WHERE realm_id = $1 AND config_type = 'stripe' AND config_key = 'api_key'"
        )
        .bind(&ctx._realm_id)
        .fetch_optional(&ctx.app_state.pool)
        .await
        ...
        let stored_secret: Option<String> = sqlx::query_scalar(
            "SELECT config_value FROM realm_config WHERE realm_id = $1 AND config_type = 'stripe' AND config_key = 'webhook_secret'"
        )
        .bind(&ctx._realm_id)
        .fetch_optional(&ctx.app_state.pool)
        .await
        ...
        assert_eq!(
            ...
            webhook_secret,
            "Webhook secret should be stored correctly"
        );
    }

    // User Story: docs/user-stories/07-payment-provider-user-stories.md
    // Covers: US-PP-003 (启用/禁用支付平台), US-PP-006 (查看支付平台使用统计)

    /// Test: Stripe webhook with invalid signature is rejected
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_stripe_webhook_signature_validation(ctx: &mut WebhookTestContext) {
        let app = ctx.create_unified_test_router();
        ...
        // Valid signature should result in OK (even if event type is unknown)
        assert_eq!(valid_response.status(), StatusCode::OK);

        // Test 2: Invalid signature should be rejected
        let invalid_response =
            crate::tests::helpers::webhook_helpers::send_stripe_webhook_with_invalid_signature(
                &app,
                &ctx._realm_id,
                payload.clone(),
            )
        ...
        assert!(
            status == StatusCode::UNAUTHORIZED || status == StatusCode::BAD_REQUEST,
            "Expected 401 or 400 for invalid signature, got {}",
            status
        );

        // Test 3: Missing signature should be rejected
        let missing_response =
            crate::tests::helpers::webhook_helpers::send_stripe_webhook_without_signature(
                &app,
                &ctx._realm_id,
                payload,
            )
        ...
        let status = missing_response.status();
        assert!(
            status == StatusCode::UNAUTHORIZED || status == StatusCode::BAD_REQUEST,
            "Expected 401 or 400 for missing signature, got {}",
            status
    ...
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_stripe_webhook_invalid_signature(ctx: &mut WebhookTestContext) {
        let app = ctx.create_unified_test_router();
        let webhook_secret = "whsec_test_webhook_secret";
        let event_id = format!("evt_test_{}", Uuid::now_v7());
        let client_app_id = Uuid::now_v7();
    ...
    /// Test: Stripe webhook without signature header is rejected
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_stripe_webhook_missing_signature_header(ctx: &mut WebhookTestContext) {
        let app = ctx.create_unified_test_router();
        let event_id = format!("evt_test_{}", Uuid::now_v7());
        let client_app_id = Uuid::now_v7();
        let plan_id = Uuid::now_v7();
    ...
    #[test_context(WebhookTestContext)]
    #[tokio::test]
    async fn test_scenario_stripe_webhook_malformed_json(ctx: &mut WebhookTestContext) {
        let app = ctx.create_unified_test_router();
        let webhook_secret = "whsec_test_webhook_secret";
        ...
        let response = app
            ...
            .oneshot(
                Request::builder()
                    .method("POST")
                    .uri(format!("/api/third/pay/{}/stripe/webhooks", ctx._realm_id))
        ...
        // Verify response is rejected (400 Bad Request)
        assert_eq!(response.status(), StatusCode::BAD_REQUEST);
    }
}
