...
mod tests {
    ...
    async fn create_test_payment_attempt(
        ...
        sqlx::query(
            "INSERT INTO payment_attempts (id, realm_id, user_id, payment_provider, target_type, target_id, amount, currency, status, expires_at)
             VALUES ($1, $2, $3, 'stripe', 'subscription_plan', $4, 5000, 'USD', 'Succeeded', NOW() + interval '1 hour')"
        )
        .bind(pa_id)
        .bind(realm_id)
        .bind(user_id)
    ...
    async fn create_test_subscription(
        ...
        sqlx::query(
            "INSERT INTO subscription (id, realm_id, external_subscription_id, external_product_id, status, tier, user_id)
    ...
    // -------------------------------------------------------------------------
    // test_user_cannot_view_other_user_invoice -- 403 for other user's invoice
    // -------------------------------------------------------------------------
    // Given: User A and User B, User B has an invoice
    // When: User A calls GET /my/invoices/{user_b_invoice_id}
    // Then: Returns 403

    #[test_context(InvoiceTestContext)]
    #[tokio::test]
    async fn test_user_cannot_view_other_user_invoice(ctx: &mut InvoiceTestContext) {
        let app = ctx.create_unified_test_router();
        let realm_id = ctx._realm_id.clone();

        // Create two regular users
        let (user_a_token, _user_a_id) =
            create_regular_user_session(ctx, "invoice-isolation-a@test.com").await;
        let (_user_b_token, user_b_id) =
            create_regular_user_session(ctx, "invoice-isolation-b@test.com").await;

        // Create invoice for User B
        let invoice_id = create_user_invoice_in_db(ctx, &realm_id, user_b_id).await;

        // User A tries to view User B's invoice
        let response = app
            .clone()
            .oneshot(
                Request::builder()
        ...
        assert_eq!(
            response.status(),
            StatusCode::FORBIDDEN,
            "Expected 403 when User A tries to view User B's invoice"
        );
    }

    // -------------------------------------------------------------------------
    // test_user_cannot_view_other_realm_invoice -- 403/404
    // -------------------------------------------------------------------------
    // Given: A user in realm-1
    // When: Trying to GET /my/invoices/{id} in a different realm
    // Then: Returns 403

    #[test_context(InvoiceTestContext)]
    #[tokio::test]
    async fn test_user_cannot_view_other_realm_invoice(ctx: &mut InvoiceTestContext) {
        let app = ctx.create_unified_test_router();
        let realm_id = ctx._realm_id.clone();

        let (user_token, user_id) =
            create_regular_user_session(ctx, "invoice-realm-isolation@test.com").await;

        // Create invoice in the user's realm
        let invoice_id = create_user_invoice_in_db(ctx, &realm_id, user_id).await;

        // Try to access the invoice via a different realm
        let wrong_realm_id = "other-realm-id";
        ...
        assert_eq!(
            response.status(),
            StatusCode::FORBIDDEN,
            "Expected 403 when accessing invoice via a different realm"
        );
    }
    ...
    // -------------------------------------------------------------------------
    // test_user_a_cannot_see_user_b_invoices -- strict applicant_user_id isolation
    // -------------------------------------------------------------------------
    // Given: User A and User B both have invoices in the same realm
    // When: User A lists their invoices
    // Then: Only User A's invoices appear (User B's invoices are excluded)

    #[test_context(InvoiceTestContext)]
    #[tokio::test]
    async fn test_user_a_cannot_see_user_b_invoices(ctx: &mut InvoiceTestContext) {
        let app = ctx.create_unified_test_router();
        let realm_id = ctx._realm_id.clone();

        // Create three users
        let (user_a_token, user_a_id) =
            create_regular_user_session(ctx, "iso-user-a@test.com").await;
        let (user_b_token, user_b_id) =
            create_regular_user_session(ctx, "iso-user-b@test.com").await;
        let (_user_c_token, user_c_id) =
            create_regular_user_session(ctx, "iso-user-c@test.com").await;

        // Create 3 invoices for User A
        let _inv_a1 = create_user_invoice_in_db(ctx, &realm_id, user_a_id).await;
        let _inv_a2 = create_user_invoice_in_db(ctx, &realm_id, user_a_id).await;
        let _inv_a3 = create_user_invoice_in_db(ctx, &realm_id, user_a_id).await;

        // Create 2 invoices for User B
        let _inv_b1 = create_user_invoice_in_db(ctx, &realm_id, user_b_id).await;
        let _inv_b2 = create_user_invoice_in_db(ctx, &realm_id, user_b_id).await;

        // Create 1 invoice for User C
        let _inv_c1 = create_user_invoice_in_db(ctx, &realm_id, user_c_id).await;

        // User A lists invoices -> should see exactly 3
        let response_a = app
            .clone()
            .oneshot(
                Request::builder()
        ...
        let body_a = parse_body(response_a.into_body()).await;
        assert_eq!(body_a["total"], 3, "User A should see exactly 3 invoices");

        // User B lists invoices -> should see exactly 2
        let response_b = app
            .clone()
            .oneshot(
                Request::builder()
        ...
        let body_b = parse_body(response_b.into_body()).await;
        assert_eq!(body_b["total"], 2, "User B should see exactly 2 invoices");
    }
