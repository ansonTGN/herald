...
mod tests {
    ...
    /// Test: 更新套餐需要 billing.manage 权限
    #[test_context(BillingTestContext)]
    #[tokio::test]
    async fn test_scenario_update_plan_requires_billing_manage_permission(
        ctx: &mut BillingTestContext,
    ) {
        let app = ctx.create_unified_test_router();
        ...
        // Step 1: 创建用户（无 billing.manage 权限）
        let (token, user_id) =
            create_admin_session_with_user(ctx, "test-user-no-perm-2@test.com", 1800).await;

        // Step 2: 创建一个测试套餐（通过 SQL 插入，跳过权限检查）
        let realm_id = ctx._realm_id.clone();
        let plan_id = create_test_plan(ctx, &realm_id, "test-plan").await;
    ...
    /// Test: 删除套餐需要 billing.manage 权限
    #[test_context(BillingTestContext)]
    #[tokio::test]
    async fn test_scenario_delete_plan_requires_billing_manage_permission(
        ctx: &mut BillingTestContext,
    ) {
        let app = ctx.create_unified_test_router();
        ...
        // Step 1: 创建用户（无 billing.manage 权限）
        let (token, user_id) =
            create_admin_session_with_user(ctx, "test-user-no-perm-3@test.com", 1800).await;
    ...
    /// Test: 分配套餐需要 billing.manage 权限
    #[test_context(BillingTestContext)]
    #[tokio::test]
    async fn test_scenario_assign_plan_requires_billing_manage_permission(
        ctx: &mut BillingTestContext,
    ) {
        let app = ctx.create_unified_test_router();
        ...
        // Step 1: 创建用户（无 billing.manage 权限）
        let (token, user_id) =
            create_admin_session_with_user(ctx, "test-user-no-perm-5@test.com", 1800).await;

        // Step 2: 创建一个测试套餐（通过 SQL 插入，跳过权限检查）
        let realm_id = ctx._realm_id.clone();
        let plan_id = create_test_plan(ctx, &realm_id, "test-plan").await;

        // Step 3: 创建一个测试 Client App（通过 SQL 插入）
        let client_app_id = Uuid::now_v7();
        let client_id = format!("test-client-{}", client_app_id);
        sqlx::query(
            "INSERT INTO client_app (id, client_id, realm_id, name, redirect_uris, created_at, updated_at)
             VALUES ($1, $2, $3, 'Test App', '[]', NOW(), NOW())",
        )
        .bind(client_app_id)
        .bind(&client_id)
        .bind(&realm_id)
        ...
        // Step 5: 验证返回 403 Forbidden
        assert_eq!(response.status(), StatusCode::FORBIDDEN);

        // Cleanup
        sqlx::query("DELETE FROM client_app WHERE id = $1")
            .bind(client_app_id)
            .execute(&ctx.app_state.pool)
            .await
            .unwrap();
    ...
    /// Test: 取消订阅需要 billing.manage 权限
    #[test_context(BillingTestContext)]
    #[tokio::test]
    async fn test_scenario_cancel_subscription_requires_billing_manage_permission(
        ctx: &mut BillingTestContext,
    ) {
        let app = ctx.create_unified_test_router();
        ...
        // Step 1: 创建用户（无 billing.manage 权限）
        let (token, user_id) =
            create_admin_session_with_user(ctx, "test-user-no-perm-6@test.com", 1800).await;

        // Step 2: 创建一个测试 Client App 和订阅（通过 SQL 插入）
        let realm_id = ctx._realm_id.clone();
        let client_app_id = Uuid::now_v7();
        let client_id = format!("test-client-{}", client_app_id);
        sqlx::query(
        ...
        // Step 4: 验证返回 403 Forbidden（或者 404 如果订阅不存在）
