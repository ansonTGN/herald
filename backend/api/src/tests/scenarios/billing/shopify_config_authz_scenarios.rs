...
mod tests {
    ...
    // ============================================================================
    // Scenario 4: Realm Admin User Can Read Config
    // ============================================================================

    /// User Story: docs/user-stories/08-shopify-pay-user-stories.md
    /// Covers: US-PP-008 (View Shopify Payment Provider configuration)
    /// Acceptance: Realm Admin can view configuration
    #[test_context(TestContext)]
    #[tokio::test]
    async fn test_scenario_shopify_config_get_with_admin_success(ctx: &mut TestContext) {
        let realm_id = ctx._realm_id.clone();
        let app = ctx.create_unified_test_router();

        // Given: Realm Admin user is logged in
        let admin_token = setup_billing_admin_session(ctx, "shopify-view@example.com").await;
        ...
        // When: GET /api/third/pay/{realmId}/providers/shopify
        let response = send_get_shopify_config_with_token(&app, &realm_id, &admin_token).await;
        ...
        // And: Sensitive fields are masked (e.g., shpat_*****)
        let admin_token_str = body["adminAccessToken"].as_str().unwrap();
        let storefront_token_str = body["storefrontAccessToken"].as_str().unwrap();
        assert!(admin_token_str.contains("****") || admin_token_str.len() < 20);
        assert!(storefront_token_str.contains("****") || storefront_token_str.len() < 20);
    }
    ...
    // ============================================================================
    // Scenario 9: Malformed JWT Token Handling
    // ============================================================================

    /// User Story: docs/user-stories/08-shopify-pay-user-stories.md
    /// Covers: US-PP-007 (Realm Admin creates and views Shopify config)
    /// Acceptance: Must verify token format
    #[test_context(TestContext)]
    #[tokio::test]
    async fn test_scenario_shopify_config_malformed_jwt_returns_401(ctx: &mut TestContext) {
        let realm_id = ctx._realm_id.clone();
        let app = ctx.create_unified_test_router();
        ...
        let malformed_tokens = vec![
            ...
            "Bearer invalid",                               // Missing signature parts
            "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.invalid", // Signature error
        ];

        for token in malformed_tokens {
            // When: Access endpoint with malformed token
            let response = send_shopify_config_request_with_token(
                &app,
                &realm_id,
                "GET",
    ...
    // ============================================================================
    // Scenario 10: Expired Session Token Handling
    // ============================================================================

    /// User Story: docs/user-stories/08-shopify-pay-user-stories.md
    /// Covers: US-PP-007 (Realm Admin creates and views Shopify config)
    /// Acceptance: Must verify session validity
    #[test_context(TestContext)]
    #[tokio::test]
    async fn test_scenario_shopify_config_expired_session_returns_401(ctx: &mut TestContext) {
        let realm_id = ctx._realm_id.clone();
        let app = ctx.create_unified_test_router();

        // Given: Test environment is ready
        // And: User had a valid session which is deleted to simulate expiration
        let (expired_token, _user_id) = crate::tests::helpers::create_admin_session_with_user(
            ctx,
        ...
        crate::application::http::auth::util::delete_session(&ctx._app_state, &expired_token)
            .await
            .expect("Failed to delete session");

        // When: Access endpoint with expired session token
        let response = send_shopify_config_request_with_token(
            &app,
            &realm_id,
            "GET",
        ...
        // And: Configuration is not accessed or modified
        let count: i64 = sqlx::query_scalar(
            "SELECT COUNT(*) FROM realm_config WHERE realm_id = $1 AND config_type = 'shopify'",
        )
        .bind(&realm_id)
        ...
        .await
        .unwrap();
        assert_eq!(count, 0); // No config was created or accessed
    }
}
