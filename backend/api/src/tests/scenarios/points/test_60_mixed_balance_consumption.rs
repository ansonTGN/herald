...
// ============================================================================
// Test 2: Subscription Credits Only Consumption
// ============================================================================

// User Story: docs/user-stories/points-billing-events.md
// Covers: US-BI-013 场景 2 - 仅会员积分消费
#[test_context(SchemaTestContext)]
#[tokio::test]
async fn test_mixed_balance_subscription_only(ctx: &mut SchemaTestContext) {
    // Given
    let realm_id = ctx._realm_id.clone();
    let user_id = create_test_user(&ctx.app_state.pool, &realm_id, "user@example.com").await;
    let plan_id = Uuid::now_v7();
    ...
    // When: Consume 3000 points (less than subscription balance)
    let identity = create_test_third_party_identity(&realm_id);
    let input = ConsumePointsInput {
        user_id: user_id.to_string(),
        client_app_id: ctx._client_app_id.clone(),
    ...
    // Verify only subscription credits consumed
    let sub_ledger = get_ledger_by_id(ctx, sub_ledger_id).await;
    assert_eq!(
        sub_ledger.remaining_amount, 2000,
        "2000 subscription credits remaining"
    );
    assert_eq!(
        sub_ledger.used_amount, 3000,
        "3000 subscription credits used"
    );

    // Verify topup credits untouched
    let topup_ledger = get_ledger_by_id(ctx, topup_ledger_id).await;
    assert_eq!(
        topup_ledger.remaining_amount, 3000,
        "Topup credits untouched"
    );
    assert_eq!(topup_ledger.used_amount, 0, "No topup credits used");

    // Verify single allocation
    let allocations = get_consumption_allocations(ctx, user_id).await;
    assert_eq!(allocations.len(), 1, "Should have 1 allocation record");
    assert_eq!(allocations[0].ledger_id, sub_ledger_id);
    assert_eq!(allocations[0].allocated_amount, 3000);
}
