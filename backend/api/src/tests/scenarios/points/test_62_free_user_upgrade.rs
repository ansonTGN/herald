...
/// ============================================================================
/// Scenario 2: Upgrade stops periodic grant schedule
/// ============================================================================
// User Story: docs/user-stories/points-free-user.md#US-FU-03
// Covers: 验收标准 3.2
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_free_user_upgrade_stops_periodic_grant(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();

    // ============================================================================
    // Given: a free user has active periodic grants
    // ============================================================================
    println!("[Step 1] Create free user with active periodic grants");
    ...
    // Create free user
    let email = "upgrade_stop_daily@example.com";
    let password = "SecurePassword123!";
    ...
    // Verify periodic grant is enabled
    let free_periodic_points_amount_before: i64 = sqlx::query_scalar(
        "SELECT free_periodic_points_amount FROM user_points_configs WHERE user_id = $1",
    )
    .bind(user_id)
    .fetch_one(&ctx._app_state.pool)
    .await
    .expect("Failed to fetch free_periodic_points_amount before");

    assert!(
        free_periodic_points_amount_before > 0,
        "Periodic grant should be enabled before upgrade"
    );

    // Verify grant schedule is active
    let schedule_active_before: bool =
        sqlx::query_scalar("SELECT active FROM points_grant_schedules WHERE user_id = $1")
            .bind(user_id)
            .fetch_one(&ctx._app_state.pool)
            .await
            .expect("Failed to fetch schedule active status before");

    assert!(
        schedule_active_before,
        "Grant schedule should be active before upgrade"
    );

    println!("[Step 1] ✓ Free user created with active periodic grants");

    // ============================================================================
    // When: the user upgrades to a paid subscription
    // ============================================================================
    println!("[Step 2] User upgrades to paid subscription");
    ...
    println!("[Step 2] ✓ User upgraded to paid subscription");
    ...
    // ============================================================================
    // Then: the user_points_configs.free_periodic_points_amount = 0
    // And: the grant_schedule.active = false
    // And: no further periodic grants will be processed
    // ============================================================================
    println!("[Step 3] Verify periodic grant schedule is stopped");

    let free_periodic_points_amount_after: i64 = sqlx::query_scalar(
        "SELECT free_periodic_points_amount FROM user_points_configs WHERE user_id = $1",
    )
    .bind(user_id)
    .fetch_one(&ctx._app_state.pool)
    .await
    .expect("Failed to fetch free_periodic_points_amount after");

    assert_eq!(
        free_periodic_points_amount_after, 0,
        "Periodic grant should be disabled after upgrade"
    );

    let schedule_active_after: bool =
        sqlx::query_scalar("SELECT active FROM points_grant_schedules WHERE user_id = $1")
            .bind(user_id)
            .fetch_one(&ctx._app_state.pool)
            .await
            .expect("Failed to fetch schedule active status after");

    assert!(
        !schedule_active_after,
        "Grant schedule should be inactive after upgrade"
    );

    println!("[Step 3] ✓ Periodic grant schedule stopped");
}

/// ============================================================================
/// Scenario 3: Upgrade creates paid subscription credits
/// ============================================================================
// User Story: docs/user-stories/points-free-user.md#US-FU-03
// Covers: 验收标准 3.3
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_free_user_upgrade_creates_subscription_credits(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();

    // ============================================================================
    // Given: a free user upgrades to "pro-monthly" plan
    // And: the plan config has: points_on_subscribe = 1000
    // ============================================================================
    println!("[Step 1] Create free user and subscription plan");
    ...
    // Create free user
    let email = "upgrade_subscription_credit@example.com";
    let password = "SecurePassword123!";
    ...
    // Create subscription plan with points_on_subscribe = 1000
    let plan_id = create_test_plan(&ctx._app_state.pool, &ctx._realm_id, "pro-monthly", 2900).await;
    let _plan_config_id =
        create_test_plan_config(&ctx._app_state.pool, &ctx._realm_id, plan_id, 1000, 30).await;

    println!("[Step 1] ✓ Free user and plan created");

    // ============================================================================
    // When: the webhook subscription.paid is processed
    // ============================================================================
    println!("[Step 2] Process subscription.paid webhook");
    ...
    // Build and send subscription.paid event (is_renewal = false for initial subscription)
    let event_id = generate_test_event_id();
    let event = build_subscription_paid_event(
        event_id.clone(),
        user_id,
        plan_id,
        false, // initial subscription, not renewal
        &ctx._realm_id,
    );
    ...
    println!("[Step 2] ✓ Subscription paid webhook processed");

    // ============================================================================
    // Then: the user receives 1000 subscription_credit points
    // And: the subscription_credit expires based on plan period
    // And: the user's total_balance = registration_credit (1000) + subscription_credit (1000)
    // ============================================================================
    println!("[Step 3] Verify subscription credits granted");

    // Verify subscription credit is granted
    ...
    assert_eq!(
        ...
        "Subscription credit should be 1000"
    );

    // Verify expiration date exists
    let expires_at: Option<chrono::DateTime<chrono::Utc>> = sqlx::query_scalar(
        "SELECT expires_at FROM points_credit_ledger WHERE user_id = $1 AND credit_type = 'subscription_credit' AND status = 'active'"
    )
    .bind(user_id)
    .fetch_one(&ctx._app_state.pool)
    .await
    .expect("Failed to fetch subscription credit expires_at");

    assert!(
        expires_at.is_some(),
        "Subscription credit should have an expiration date"
    );

    let expiration_date = expires_at.unwrap();
    let now = chrono::Utc::now();
    let expected_min = now + chrono::Duration::days(29);
    let expected_max = now + chrono::Duration::days(31);

    assert!(
        expiration_date >= expected_min && expiration_date <= expected_max,
        "Expiration date should be approximately 30 days from now"
    );
    ...
    println!("[Step 3] ✓ Subscription credits granted with proper expiration");
}
...
/// ============================================================================
/// Scenario 5: Re-upgrade after cancellation
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_free_user_re_upgrade_after_cancellation(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();

    // ============================================================================
    // Given: a user previously upgraded and then cancelled
    // And: the user has 1000 registration_credit
    // ============================================================================
    println!("[Step 1] Create user, upgrade, then cancel");
    ...
    // Create user
    let email = "reupgrade_user@example.com";
    let password = "SecurePassword123!";
    ...
    // Create and cancel first subscription via webhook
    let plan_id = create_test_plan(&ctx._app_state.pool, &ctx._realm_id, "pro-monthly", 2900).await;
    let _plan_config_id =
        create_test_plan_config(&ctx._app_state.pool, &ctx._realm_id, plan_id, 1000, 30).await;
    ...
    // Build and send first subscription.paid event
    let event_id1 = generate_test_event_id();
    let event1 = build_subscription_paid_event(
        event_id1.clone(),
        user_id,
        plan_id,
        false, // initial subscription
    ...
    let webhook_response1 =
        send_webhook_with_signature(&app, &ctx._realm_id, event1, "test_webhook_secret").await;
    assert_eq!(
        webhook_response1.status(),
        StatusCode::OK,
        "Webhook should succeed"
    );

    // Get first subscription_id from database for cancellation
    // Note: subscription table doesn't have user_id, so we query by realm_id and status
    let _subscription_id1: String = sqlx::query_scalar(
        "SELECT id::text FROM subscription WHERE realm_id = $1 AND status = 'active' ORDER BY created_at DESC LIMIT 1"
    )
    .bind(&ctx._realm_id)
    .fetch_one(&ctx._app_state.pool)
    .await
    .expect("Failed to fetch subscription_id");
    let _subscription_id1 =
        uuid::Uuid::parse_str(&_subscription_id1).expect("Invalid subscription ID");
    ...
    println!("[Step 1] ✓ User upgraded and cancelled");

    // ============================================================================
    // When: the user subscribes to a paid plan again
    // ============================================================================
    println!("[Step 2] User re-upgrades to paid plan");

    // Build and send second subscription.paid event
    let event_id2 = generate_test_event_id();
    let event2 = build_subscription_paid_event(
        event_id2.clone(),
        user_id,
        plan_id,
        false, // initial subscription
    ...
    let webhook_response2 =
        send_webhook_with_signature(&app, &ctx._realm_id, event2, "test_webhook_secret").await;
    assert_eq!(
        webhook_response2.status(),
        StatusCode::OK,
        "Webhook should succeed"
    );

    println!("[Step 2] ✓ User re-upgraded to paid plan");

    // ============================================================================
    // Then: the registration_credit (1000) continues to be preserved
    // And: new subscription credits are granted
    // And: the user's total_balance = registration + new_subscription
    // ============================================================================
    println!("[Step 3] Verify re-upgrade results");
    ...
    // Verify new subscription credits are granted
    let subscription_balance: i64 = sqlx::query_scalar(
        "SELECT COALESCE(CAST(SUM(granted_amount) AS BIGINT), 0) FROM points_credit_ledger WHERE user_id = $1 AND credit_type = 'subscription_credit' AND status = 'active'"
    )
    .bind(user_id)
    ...
    assert_eq!(
        subscription_balance, 1000,
        "New subscription credit should be 1000"
    );
    ...
    assert_eq!(
        total_balance, 2000,
        "Total balance should be 2000 (1000 registration + 1000 new subscription)"
    );

    println!(
        "[Step 3] ✓ Re-upgrade verified: registration preserved, new subscription credits granted"
    );
}
