...
// =============================================================================
// Test 75.8: Wallet auto-created on first grant
// =============================================================================
// User Story: US-TP-017
// Covers: Target user has no wallet, grant auto-creates it with correct balances
// =============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_75_8_wallet_auto_created_on_first_grant(ctx: &mut TestContext) {
    // Given: Target user exists but has no points_wallets row
    let (api_key, _) = create_ext_api_key(ctx).await;
    let target_user_id = create_target_user(ctx, "target_75_8@test.com").await;
    ...
    // When: POST grant with amount=75
    let (status, body) = grant_points_ext_via_api(
        ctx,
        &ctx._realm_id,
        target_user_id,
    ...
    assert_eq!(status, StatusCode::OK, "Expected 200 OK, got {:?}", status);
    let resp = body.expect("Response body should exist");
    assert_eq!(resp["amount"], 75);
    assert_eq!(resp["grantedBalance"], 75);
