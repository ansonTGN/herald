...
// ============================================================================
// Scenario 4: Missing userId field should be rejected
// ============================================================================
// User Story: docs/user-stories/points-billing-events.md
// Covers: Required field validation - userId must be present

#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_consume_missing_user_id_rejected(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();

    // Given: valid API key (no user setup needed; rejection happens before lookup)
    let client_app_id = create_test_client_app(&ctx._app_state.pool, &ctx._realm_id).await;
    let api_key = create_test_api_key(&ctx._app_state.pool, &ctx._realm_id, client_app_id).await;

    // When: payload without userId
    let payload = json!({
        "clientAppId": client_app_id.to_string(),
        "amount": 100,
    ...
    // Then: 422 Unprocessable Entity (JSON deserialization failure for missing required field)
    // or 400 Bad Request depending on axum Json extractor behavior
    assert!(
        response.status() == StatusCode::UNPROCESSABLE_ENTITY
            || response.status() == StatusCode::BAD_REQUEST,
        "Missing userId should be rejected, got: {}",
        response.status()
    );
}

// ============================================================================
// Scenario 5: Missing amount field should be rejected
// ============================================================================
// User Story: docs/user-stories/points-billing-events.md
// Covers: Required field validation - amount must be present

#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_consume_missing_amount_rejected(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();

    // Given: valid API key and a user ID
    let user_id = create_test_user(
        &ctx._app_state.pool,
        &ctx._realm_id,
        "edge-no-amount@example.com",
    )
    .await;
    let client_app_id = create_test_client_app(&ctx._app_state.pool, &ctx._realm_id).await;
    let api_key = create_test_api_key(&ctx._app_state.pool, &ctx._realm_id, client_app_id).await;

    // When: payload without amount
    let payload = json!({
        "userId": user_id.to_string(),
        "clientAppId": client_app_id.to_string(),
    ...
    assert!(
        response.status() == StatusCode::UNPROCESSABLE_ENTITY
            || response.status() == StatusCode::BAD_REQUEST,
        "Missing amount should be rejected, got: {}",
        response.status()
    );
}
