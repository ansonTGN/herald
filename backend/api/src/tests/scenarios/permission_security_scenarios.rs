...
mod tests {
    ...
    ///
    /// **Given**: 管理员用户拥有所有权限
    /// **And**: 存在内置权限 `users.manage` (is_builtin = true)
    /// **When**: 用户尝试删除该内置权限
    /// **Then**: API 返回 403 Forbidden
    /// **And**: 错误消息是 "Cannot delete built-in permission"
    #[test_context(PermissionSecurityTestContext)]
    #[tokio::test]
    async fn test_scenario_cannot_delete_builtin_permission(
        ctx: &mut PermissionSecurityTestContext,
    ) {
        // ========================================================================
        // Given: 创建管理员用户
        // ========================================================================
        let (admin_token, _user_id_str) =
            create_admin_session_with_user(ctx, "test-admin-builtin@test.com", 1800).await;
        grant_realm_admin_role(ctx, &_user_id_str).await;

        // 创建一个内置权限
        let permission_id = uuid::Uuid::now_v7();
        sqlx::query(
            "INSERT INTO permissions (id, name, resource, action, description, realm_id, is_builtin, created_at, updated_at)
             VALUES ($1, $2, $3, $4, $5, $6, $7, NOW(), NOW())",
        )
        .bind(permission_id)
        .bind("test.builtin.manage")
        .bind("test")
        .bind("manage")
        .bind("Built-in test permission")
        .bind(&ctx._realm_id)
        .bind(true) // is_builtin = true
        .execute(&ctx._app_state.pool)
        .await
        .expect("Failed to create builtin permission");
        ...
        assert_eq!(
            delete_response.status(),
            StatusCode::FORBIDDEN,
            "Delete should be forbidden for builtin permission"
        );
        ...
        assert!(
            error_message.contains("built-in") || error_message.contains("builtin"),
            "Error message should mention built-in permission, got: {}",
            error_message
        );
        ...
        tracing::info!("✓ Builtin permission correctly protected from deletion");
    }
    ...
    /// **When**: 用户尝试删除该内置角色
    /// **Then**: API 返回 403 Forbidden
    /// **And**: 错误消息是 "Cannot delete built-in role"
    #[test_context(PermissionSecurityTestContext)]
    #[tokio::test]
    async fn test_scenario_cannot_delete_builtin_role(ctx: &mut PermissionSecurityTestContext) {
        // ========================================================================
        // Given: 创建管理员用户
        // ========================================================================
        let (admin_token, _user_id_str) =
            create_admin_session_with_user(ctx, "test-admin-builtin-role@test.com", 1800).await;
        grant_realm_admin_role(ctx, &_user_id_str).await;
        ...
        .bind(role_id)
        .bind("test-builtin-admin")
        .bind("Built-in test role")
        .bind(&ctx._realm_id)
        .bind(&ctx._client_id)
        .bind(true) // is_builtin = true
        .execute(&ctx._app_state.pool)
        .await
        .expect("Failed to create builtin role");
        ...
        assert_eq!(
            delete_response.status(),
            StatusCode::FORBIDDEN,
            "Delete should be forbidden for builtin role"
        );
        ...
        assert!(
            error_message.contains("built-in") || error_message.contains("builtin"),
            "Error message should mention built-in role, got: {}",
            error_message
        );
        ...
        tracing::info!("✓ Builtin role correctly protected from deletion");
    }

    /// 场景测试：View 权限不能删除
    ///
    /// **Given**: 用户拥有 `permissions:view` 或 `roles:view` 权限
    /// **When**: 用户尝试删除任何权限/角色定义
    /// **Then**: API 返回 403 Forbidden
    /// **And**: 层级不适用（view 不包含 manage）
    #[test_context(PermissionSecurityTestContext)]
    #[tokio::test]
    async fn test_scenario_view_permission_cannot_delete(ctx: &mut PermissionSecurityTestContext) {
        // 这个测试验证权限层级不会意外授予删除权限
        // 如果用户只有 view 权限，即使层级系统认为 view < manage
        // 删除操作仍然需要明确的 manage 权限

        // ========================================================================
        // Given: 创建用户并授予 permissions:view 权限
        // ========================================================================
        let (admin_token, user_id_str) =
            create_admin_session_with_user(ctx, "test-view-only@test.com", 1800).await;
        let _user_id = uuid::Uuid::parse_str(&user_id_str).expect("Invalid user_id UUID");

        // 临时授予 realm-admin 角色来创建测试角色
        grant_realm_admin_role(ctx, &user_id_str).await;

        let role_name = "test-view-only-role";
        let role_id = create_role(
            ctx,
            &ctx._realm_id,
            &admin_token,
            role_name,
            "Role with view permissions only",
        )
        .await;
        ...
        let delete_req = Request::builder()
            ...
            .uri(format!(
                "/api/permission/{}/define/{}",
                ctx._realm_id,
                uuid::Uuid::now_v7() // 不存在的 ID，应该先检查权限
            ))
            .header(header::COOKIE, format!("X-Auth={}", admin_token))
            .body(Body::empty())
            .unwrap();
        ...
        // ========================================================================
        // Then: 验证返回 403 Forbidden（权限检查在资源查找之前）
        // ========================================================================
        assert_eq!(
            delete_response.status(),
            StatusCode::FORBIDDEN,
            "Delete should be forbidden with view-only permission"
        );

        tracing::info!("✓ View permission correctly does not grant delete access");
    }

    #[test_context(PermissionSecurityTestContext)]
    #[tokio::test]
    async fn test_scenario_delete_permission_endpoint_requires_policies_manage(
        ctx: &mut PermissionSecurityTestContext,
    ) {
        let (token, _user_id) =
            create_admin_session_with_user(ctx, "test-delete-policy@test.com", 1800).await;
        ...
        assert!(
            message.contains("policies") && message.contains("manage"),
            "Error should mention policies.manage, got: {}",
            message
        );
    }
}
