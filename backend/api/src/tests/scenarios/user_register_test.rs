...
async fn test_scenario_user_register_success(ctx: &mut TestContext) {
    ...
    // Step 2: 使用有效的邮箱和密码注册
    // ============================================================================
    println!("[Step 2] 用户注册: test-register@cas.com");
    let email = "test-register@cas.com";
    ...
    println!("[Step 2] ✓ 注册请求完成");

    // ============================================================================
    // Step 3: 验证响应状态码为 200
    // ============================================================================
    println!("[Step 3] 验证响应状态码");
    assert_eq!(
        response.status(),
        StatusCode::OK,
        "Registration should return 200 OK"
    );
    println!("[Step 3] ✓ 响应状态码为 200");

    // ============================================================================
    // Step 4: 验证返回的消息包含验证要求
    // ============================================================================
    println!("[Step 4] 验证响应 JSON");
    ...
    assert_eq!(
        ...
        "verification_required should be true"
    );
    assert!(
        body["message"].as_str().unwrap().contains("verify"),
        "Message should mention email verification"
    );
    println!(
        "[Step 4] ✓ 响应正确: verification_required={}, message={}",
        body["verificationRequired"], body["message"]
    );

    // ============================================================================
    // Step 5: 验证用户已创建且状态为 0（等待验证）
    // ============================================================================
    println!("[Step 5] 验证用户状态");
    ...
    assert_eq!(status, 0, "User status should be 0 (wait verified)");
    println!(
        "[Step 5] ✓ 用户创建成功: user_id={}, status={}",
        user_id, status
    );
    ...
    assert_eq!(code_count, 1, "Should have one verification code");
    println!("[Step 6] ✓ 邮箱验证码已生成");
    ...
    println!("\n✅ User Story 完成：用户注册成功");
}

/// ============================================================================
/// User Story: 注册后未验证邮箱无法登录
///
/// **场景描述**：
/// 用户注册成功后，在未验证邮箱的情况下尝试登录，系统应该拒绝访问。
///
/// **测试步骤**：
/// 1. 启用注册功能并注册新用户
/// 2. 验证用户状态为 0（未激活）
/// 3. 尝试使用注册的邮箱和密码登录
/// 4. 验证返回 403 Forbidden
///
/// **验收标准**：
/// - 返回 403 Forbidden
...
async fn test_scenario_user_register_cannot_login_without_verification(ctx: &mut TestContext) {
    ...
    // ============================================================================
    // Step 2: 验证用户状态为 0（未激活）
    // ============================================================================
    println!("[Step 2] 验证用户状态");
    ...
    println!("[Step 2] ✓ 用户状态为 0（未激活）");

    // ============================================================================
    // Step 3: 尝试使用注册的邮箱和密码登录
    // ============================================================================
    println!("[Step 3] 尝试登录（未验证邮箱）");
    ...
    // ============================================================================
    // Step 4: 验证返回 403 Forbidden
    // ============================================================================
    println!("[Step 4] 验证返回 403 Forbidden");
    assert_eq!(
        login_response.status(),
        StatusCode::FORBIDDEN,
        "Should return 403 Forbidden for unverified user"
    );
    ...
    assert!(
        response_message(&body).contains("email not verified or account not active"),
        "Error message should mention inactive account"
    );

    println!("[Step 4] ✓ 返回 403 Forbidden，错误信息正确");
...
/// ============================================================================
/// User Story: 需要邮箱验证时注册创建待激活用户
///
/// **场景描述**：
/// 当 realm 配置 `require_email_verification = true` 时，注册应该：
/// - 创建状态为 0（待验证）的用户
/// - 生成邮箱验证码
/// - 返回 verification_required: true
///
/// **测试步骤**：
/// 1. 启用注册功能
/// 2. 启用邮箱验证要求 (require_email_verification = true)
/// 3. 注册新用户
/// 4. 验证用户状态为 0
/// 5. 验证邮箱验证码已生成
/// 6. 验证响应包含 verification_required: true
///
/// **验收标准**：
/// - 用户状态为 0（待验证）
/// - 邮箱验证码存在于数据库
/// - verification_required 为 true
/// - 消息提示需要验证邮箱
///
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_user_register_with_verification_required
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_user_register_with_verification_required(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    // Step 2: 启用邮箱验证要求
    // ============================================================================
    println!("[Step 2] 启用邮箱验证要求 (require_email_verification = true)");
    sqlx::query(
        "INSERT INTO realm_config (realm_id, config_type, config_key, config_value, enabled)
         VALUES ($1, 'registration', 'require_email_verification', 'true', true)",
    )
    ...
    .await
    .expect("Failed to enable email verification requirement");
    println!("[Step 2] ✓ 邮箱验证要求已启用");

    // ============================================================================
    // Step 3: 注册新用户
    // ============================================================================
    println!("[Step 3] 注册新用户: verify-required@cas.com");
    let email = "verify-required@cas.com";
    let password = "password123";
    ...
    assert_eq!(status, 0, "User status should be 0 (pending verification)");
    println!(
        "[Step 4] ✓ 用户状态正确: user_id={}, status=0 (pending)",
        user_id
    );
    ...
    assert_eq!(code_count, 1, "Should have one verification code");
    println!("[Step 5] ✓ 邮箱验证码已生成: count={}", code_count);

    // ============================================================================
    // Step 6: 验证响应包含 verification_required: true
    // ============================================================================
    ...
    println!(
        "[Step 6] ✓ 响应正确: verification_required=true, message={}",
        body["message"]
    );
    ...
    println!("\n✅ User Story 完成：需要邮箱验证时注册创建待激活用户");
}
...
/// ============================================================================
/// User Story: 未配置邮箱验证时默认不需要验证
///
/// **场景描述**：
/// 当 realm 未配置 `require_email_verification` 时，注册应该：
/// - 创建状态为 1（已激活）的用户
/// - 不生成邮箱验证码
/// - 返回 verification_required: false（默认行为）
///
/// **测试步骤**：
/// 1. 启用注册功能
/// 2. 不配置 require_email_verification（测试默认行为）
/// 3. 注册新用户
/// 4. 验证用户状态为 1（已激活）
/// 5. 验证邮箱验证码未生成
/// 6. 验证响应包含 verification_required: false
...
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_user_register_default_no_verification
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_user_register_default_no_verification(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();

    // ============================================================================
    // Step 1: 启用注册功能（不配置 require_email_verification）
    // ============================================================================
    println!("[Step 1] 启用注册功能（未配置 require_email_verification）");
    sqlx::query(
        "INSERT INTO realm_config (realm_id, config_type, config_key, config_value, enabled)
         VALUES ($1, 'registration', 'allowed', 'true', true)",
    )
    ...
    .await
    .expect("Failed to enable registration");
    println!("[Step 1] ✓ 注册功能已启用（邮箱验证未配置）");

    // ============================================================================
    // Step 2: 注册新用户
    // ============================================================================
    println!("[Step 2] 注册新用户: default-no-verify@cas.com");
    let email = "default-no-verify@cas.com";
    let password = "password123";
    ...
    // ============================================================================
    // Step 3: 验证用户状态为 1（已激活，默认行为）
    // ============================================================================
    println!("[Step 3] 验证用户状态（默认不需要验证）");
    let (user_id, status): (String, i16) =
        sqlx::query_as("SELECT id::text, status FROM account WHERE realm_id = $1 AND email = $2")
            .bind(&ctx._realm_id)
            .bind(email)
    ...
    assert_eq!(status, 1, "User status should be 1 (active) by default");
    println!(
        "[Step 3] ✓ 用户状态正确: user_id={}, status=1 (active, default)",
        user_id
    );
    ...
    assert_eq!(code_count, 0, "Should have no verification code by default");
    println!("[Step 4] ✓ 邮箱验证码未生成: count=0");

    // ============================================================================
    // Step 5: 验证响应包含 verification_required: false
    // ============================================================================
    println!("[Step 5] 验证响应 JSON");
    let body_bytes = axum::body::to_bytes(response.into_body(), usize::MAX)
        .await
        .expect("Failed to read response body");
    let body: serde_json::Value =
    ...
    assert_eq!(
        body["verificationRequired"], false,
        "verification_required should be false by default"
    );
    println!(
        "[Step 5] ✓ 响应正确: verification_required=false (default), message={}",
        body["message"]
    );
    ...
    println!("\n✅ User Story 完成：未配置邮箱验证时默认不需要验证");
}

/// ============================================================================
/// User Story: 场景 1a - 注册成功不需要邮箱验证
///
/// **场景描述**：
/// 当 realm 配置 `require_email_verification = false` 或未设置时，注册应该：
/// - 创建状态为 1（已激活）的用户
/// - 不生成邮箱验证码
/// - 返回 verification_required: false
/// - 用户可以立即登录
...
/// **测试步骤**：
/// 1. 启用 realm 的注册功能
/// 2. 配置 require_email_verification = false
/// 3. 注册新用户
/// 4. 验证用户状态为 1（已激活）
/// 5. 验证邮箱验证码未生成
/// 6. 验证响应包含 verification_required: false
...
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_user_register_success_without_verification
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_user_register_success_without_verification(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    // ============================================================================
    // Step 2: 配置 require_email_verification = false
    // ============================================================================
    println!("[Step 2] 配置不需要邮箱验证 (require_email_verification = false)");
    sqlx::query(
        "INSERT INTO realm_config (realm_id, config_type, config_key, config_value, enabled)
         VALUES ($1, 'registration', 'require_email_verification', 'false', true)",
    )
    ...
    // Step 3: 注册新用户
    // ============================================================================
    println!("[Step 3] 用户注册: no-verify-success@cas.com");
    let email = "no-verify-success@cas.com";
    let password = "password123";
    ...
    assert_eq!(status, 1, "User status should be 1 (active)");
    println!(
        "[Step 4] ✓ 用户创建成功: user_id={}, status=1 (active)",
        user_id
    );
    ...
    println!("\n✅ User Story 场景 1a 完成：注册成功不需要邮箱验证");
}
...
/// ============================================================================
/// User Story: 场景 7 - Turnstile 验证失败
///
/// **场景描述**：
/// 当 realm 开启了 Turnstile 验证时，用户未完成 Turnstile 验证就提交注册表单，系统应该拒绝请求。
///
/// **测试步骤**：
/// 1. 启用 realm 的注册功能
/// 2. 启用 Turnstile 验证（配置 site_secret）
/// 3. 尝试注册时不提供 turnstile_token 或提供无效 token
/// 4. 验证返回 400 Bad Request 或 401 Unauthorized
/// 5. 验证错误消息包含 "turnstile" 或 "captcha" 相关提示
///
/// **验收标准**：
/// - 返回 400 Bad Request 或 401 Unauthorized
/// - 错误消息包含 "turnstile" 或 "captcha" 关键词
///
/// **API 端点**：
...
/// **运行方式**：
/// ```bash
/// cargo nextest run test_scenario_turnstile_verification_failure
/// ```
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_turnstile_verification_failure(ctx: &mut TestContext) {
    let app = ctx.create_unified_test_router();
    ...
    println!("[Step 1] ✓ 注册功能已启用");

    // ============================================================================
    // Step 2: 启用 Turnstile 验证（配置 site_secret）
    // ============================================================================
    println!("[Step 2] 启用 Turnstile 验证");
    sqlx::query(
        "INSERT INTO realm_config (realm_id, config_type, config_key, config_value, enabled, metadata)
         VALUES ($1, 'turnstile', 'site_secret', 'test_secret', true, '{\"site_key\": \"test_site_key\"}'::jsonb)",
    )
    .bind(&ctx._realm_id)
    .execute(&ctx._app_state.pool)
    .await
    .expect("Failed to enable Turnstile");

    println!("[Step 2] ✓ Turnstile 验证已启用");

    // ============================================================================
    // Step 3: 尝试注册时不提供 turnstile_token
    // ============================================================================
    println!("[Step 3] 尝试注册（未提供 Turnstile token）");
    let email = "turnstile-fail@cas.com";
    let password = "password123";

    let payload = json!({
        "email": email,
        "password": password
        // Note: turnstileToken is NOT provided
    });
    ...
    // ============================================================================
    // Step 4: 验证返回 400 Bad Request 或 401 Unauthorized
    // ============================================================================
    println!("[Step 4] 验证响应状态码");
    assert!(
        response.status() == StatusCode::BAD_REQUEST
            || response.status() == StatusCode::UNAUTHORIZED,
        "Should return 400 Bad Request or 401 Unauthorized for missing Turnstile token"
    );
    println!("[Step 4] ✓ 返回状态码: {}", response.status());

    // ============================================================================
    // Step 5: 验证错误消息包含 "turnstile" 或 "captcha" 相关提示
    // ============================================================================
    println!("[Step 5] 验证错误消息");
    let body_bytes = axum::body::to_bytes(response.into_body(), usize::MAX)
        .await
        .expect("Failed to read response body");
    let body: serde_json::Value =
    ...
    let error_message = response_message(&body);

    assert!(
        error_message.to_lowercase().contains("turnstile")
            || error_message.to_lowercase().contains("captcha"),
        "Error message should mention 'turnstile' or 'captcha', got: {}",
        error_message
    );
    println!("[Step 5] ✓ 错误消息正确: {}", error_message);
    ...
    assert!(
        !user_exists,
        "User should not be created when Turnstile verification fails"
    );
    println!("[Step 5] ✓ 用户未被创建");

    println!("\n✅ User Story 场景 7 完成：Turnstile 验证失败");
}
