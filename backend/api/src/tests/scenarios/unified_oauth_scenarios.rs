...
/// ============================================================================
/// Scenario 5: OAuth Callback Handling - New User Creation
///
/// **Given**:
/// - User initiated OAuth and received authorization URL
/// - User has clicked URL and authorized application
...
///
/// **Then**:
/// - Returns 200 OK with JWT token (or error without mock server)
/// - New user account is created in database (with mock server)
/// - User's OAuth provider is linked (with mock server)
/// - User email is verified (with mock server)
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_oauth_callback_new_user(ctx: &mut TestContext) {
    for provider_config in all_oauth_providers() {
        let realm_id = ctx._realm_id.clone();
        let provider_type = provider_config.provider_type;

        println!("\n[Scenario 5] Testing {} callback new user", provider_type);

        // ============================================================================
        // Step 1: Create Provider configuration
        // ============================================================================
        println!("[Step 1] Creating {} Provider configuration", provider_type);
        let admin_email = provider_config.admin_email_for("callback-new");
        let (token, admin_user_id) = create_admin_session_with_user(ctx, &admin_email, 1800).await;
        grant_realm_admin_role(ctx, &admin_user_id).await;
        ...
        // ============================================================================
        // Step 2: Generate authorization URL and extract state
        // ============================================================================
        println!("[Step 2] Generating authorization URL");
        let (_, state_token) =
            generate_auth_url_and_extract_state(ctx, &realm_id, provider_type).await;
        println!("[Step 2] ✓ State token: {}", state_token);

        // ============================================================================
        // Step 3: Simulate OAuth callback with mock authorization code
        // ============================================================================
        println!("[Step 3] Simulating OAuth callback");
        let mock_auth_code = format!("mock_{}_auth_code_12345", provider_type);
        let test_email = format!("{}.new.user@example.com", provider_type);

        let callback_response =
            send_oauth_callback(ctx, &realm_id, provider_type, &mock_auth_code, &state_token).await;

        // ============================================================================
        // Step 4: Verify response (may fail without real mock server)
        // ============================================================================
        match callback_response {
            Ok(response) => {
                println!("[Step 4] Callback response status: {}", response.status());
                ...
                if response.status() == StatusCode::OK {
                    ...
                    // ============================================================================
                    println!("[Step 5] Verifying user creation");

                    let user_exists = user_exists(ctx, test_email.as_str(), &realm_id).await;

                    if user_exists {
                        println!("[Step 5] ✓ User created with email: {}", test_email);

                        // ============================================================================
                        // Step 6: Verify OAuth provider linked
                        // ============================================================================
                        println!("[Step 6] Verifying OAuth provider link");

                        let provider_linked =
                            oauth_provider_linked(ctx, &realm_id, provider_type).await;

                        assert!(
                            provider_linked,
                            "{} provider should be linked to user",
                            provider_type
                        );
                        println!("[Step 6] ✓ {} provider linked to user", provider_type);

                        // ============================================================================
                        // Step 7: Verify JWT token in response
                        // ============================================================================
                        println!("[Step 7] Verifying JWT token");

                        assert!(
                            json.get("access_token").is_some() || json.get("token").is_some(),
                            "Response should contain access_token or token"
                        );
                        println!("[Step 7] ✓ JWT token present in response");
                    } else {
                        println!("[Step 5] Note: User creation requires real mock server response");
                        println!("[Step 5] This is expected in isolated test environment");
                    }
                } else {
                    println!("[Step 4] Callback failed (expected without real mock server)");
                    println!("[Step 4] Status: {}", response.status());
                }
            }
            Err(e) => {
                println!("[Step 4] Callback request failed: {}", e);
                println!("[Step 4] This is expected without real Beeceptor mock server");
            }
        }
    }
}
...
/// ============================================================================
/// Scenario 6: OAuth Callback Handling - Existing User Login
///
/// **Given**:
/// - User account already exists in database
/// - User's OAuth provider is linked
/// - User initiates OAuth flow
///
/// **When**:
/// - OAuth provider redirects callback with valid code and state
///
/// **Then**:
/// - Returns 200 OK with JWT token (or error without mock server)
/// - No new user account is created (with mock server)
/// - Existing user's information is used (with mock server)
/// ============================================================================
#[test_context(TestContext)]
#[tokio::test]
async fn test_scenario_oauth_callback_existing_user(ctx: &mut TestContext) {
    for provider_config in all_oauth_providers() {
        let realm_id = ctx._realm_id.clone();
        let provider_type = provider_config.provider_type;

        println!(
            "\n[Scenario 6] Testing {} callback existing user",
            provider_type
        );

        // ============================================================================
        // Step 1: Create admin session and configure OAuth
        // ============================================================================
        println!(
            "[Step 1] Creating admin session and {} OAuth config",
            provider_type
        );
        let admin_email = provider_config.admin_email_for("callback-existing");
        let (token, admin_user_id) = create_admin_session_with_user(ctx, &admin_email, 1800).await;
        grant_realm_admin_role(ctx, &admin_user_id).await;
        ...
        // ============================================================================
        println!("[Step 2] Creating existing user");
        let test_email = format!("{}.existing.user@example.com", provider_type);

        create_existing_user_with_oauth_link(
            ctx,
            &realm_id,
            test_email.as_str(),
            provider_type,
            format!("{}_user_123", provider_type).as_str(),
            format!("{}_open_456", provider_type).as_str(),
        )
        .await;

        println!("[Step 2] ✓ Existing user created: {}", test_email);
        ...
        let (_, state_token) =
            generate_auth_url_and_extract_state(ctx, &realm_id, provider_type).await;
        println!("[Step 3] ✓ State token: {}", state_token);

        // ============================================================================
        // Step 4: Count users before callback
        // ============================================================================
        println!("[Step 4] Counting users before callback");
        let user_count_before = count_users(ctx, &realm_id).await;
        println!("[Step 4] ✓ User count before: {}", user_count_before);

        // ============================================================================
        // Step 5: Simulate OAuth callback
        // ============================================================================
        println!("[Step 5] Simulating OAuth callback for existing user");
        let mock_auth_code = format!("mock_{}_auth_code_existing", provider_type);

        let callback_response =
            send_oauth_callback(ctx, &realm_id, provider_type, &mock_auth_code, &state_token).await;

        // ============================================================================
        // Step 6: Verify no new user was created
        // ============================================================================
        match callback_response {
            Ok(response) => {
                let user_count_after = count_users(ctx, &realm_id).await;

                assert_eq!(
                    user_count_before, user_count_after,
                    "No new user should be created for existing user ({} provider)",
                    provider_type
                );
                println!("[Step 6] ✓ No new user created (count unchanged)");
                ...
                if response.status() == StatusCode::OK {
                    ...
                    println!("[Step 6] ✓ Existing user logged in successfully");
                }
            }
            Err(e) => {
                println!("[Step 5] Callback failed: {}", e);
                println!("[Step 5] This is expected without real mock server");
            }
        }
    }
}
