...
mod tests {
    ...
    #[test]
    fn test_wechat_order_status_as_str() {
        assert_eq!(WechatOrderStatus::Pending.as_str(), "pending");
        assert_eq!(WechatOrderStatus::Paid.as_str(), "paid");
        assert_eq!(WechatOrderStatus::Closed.as_str(), "closed");
        assert_eq!(WechatOrderStatus::Expired.as_str(), "expired");
    }

    #[test]
    fn test_wechat_order_status_from_str() {
        assert_eq!(
            "pending".parse::<WechatOrderStatus>(),
            Ok(WechatOrderStatus::Pending)
        );
        assert_eq!(
            "paid".parse::<WechatOrderStatus>(),
            Ok(WechatOrderStatus::Paid)
        );
        assert_eq!(
            "closed".parse::<WechatOrderStatus>(),
            Ok(WechatOrderStatus::Closed)
        );
        assert_eq!(
            "expired".parse::<WechatOrderStatus>(),
            Ok(WechatOrderStatus::Expired)
        );
        assert!("unknown".parse::<WechatOrderStatus>().is_err());
    }

    #[test]
    fn test_wechat_order_status_display() {
        assert_eq!(format!("{}", WechatOrderStatus::Pending), "pending");
        assert_eq!(format!("{}", WechatOrderStatus::Paid), "paid");
    }

    #[test]
    fn test_generate_out_trade_no_format() {
    ...
    #[test]
    fn test_generate_out_trade_no_short_realm() {
        let realm_id = "ab";
        let out_trade_no = generate_out_trade_no(realm_id);
    ...
    #[test]
    fn test_generate_out_trade_no_uniqueness() {
        ...
        assert_ne!(first, second, "Each out_trade_no should be unique");
    }
}
