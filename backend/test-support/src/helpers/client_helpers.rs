...
use crate::helpers::billing_helpers::ensure_test_bucket_for_realm;
...
pub async fn create_third_party_test_subscription(
    ...
    let bucket_id = ensure_test_bucket_for_realm(&ctx.app_state.pool, &ctx._realm_id).await;
...
pub async fn clear_api_key_cache(ctx: &TestContext) {
    // 删除所有 api_key:* 键
    let redis_manager = ctx.app_state.redis_manager.clone();
