...
/// * `user_id` - 用户ID
/// * `provider_type` - 提供者类型
/// * `provider_user_id` - 提供者端的用户ID
///
/// # Returns
/// 如果链接存在则返回true，否则返回false
pub async fn verify_oauth_provider_linked(
    ctx: &TestContext,
    user_id: &str,
    provider_type: &str,
    ...
    if linked {
        tracing::debug!(
            ...
            "Verified OAuth provider linked"
        );
    } else {
        tracing::debug!(
            user_id = %user_id,
...
///
/// # Returns
/// 提供者绑定列表，每个元素为 (provider_type, provider_user_id, open_id, union_id)
pub async fn get_user_oauth_providers(
    ctx: &TestContext,
    user_id: &str,
) -> Vec<(String, String, Option<String>, Option<String>)> {
    let providers = sqlx::query_as::<_, (String, String, Option<String>, Option<String>)>(
        r#"
        SELECT provider_type, provider_user_id, open_id, union_id
        FROM user_oauth_providers
    ...
    tracing::debug!(
        ...
        "Retrieved {} OAuth providers for user",
        providers.len()
    );
...
/// * `ctx` - 测试上下文
/// * `user_id` - 用户ID
pub async fn cleanup_user_oauth_links(ctx: &TestContext, user_id: &str) {
    let result =
        sqlx::query("DELETE FROM user_oauth_providers WHERE user_id = $1 AND realm_id = $2")
            .bind(user_id)
            .bind(&ctx._realm_id)
            .execute(&ctx.app_state.pool)
            .await;
    ...
    match result {
        Ok(rows_affected) => {
            tracing::debug!(
                user_id = %user_id,
                deleted_count = rows_affected.rows_affected(),
                "Cleaned up user OAuth links"
            );
        }
        Err(e) => {
            tracing::warn!(
                user_id = %user_id,
                error = %e,
                "Failed to clean up user OAuth links"
            );
        }
    }
}
...
/// 清理测试用的OAuth用户
///
/// 删除通过OAuth创建的测试用户（注意：这会删除用户，不仅仅是OAuth链接）。
///
/// # Arguments
/// * `ctx` - 测试上下文
/// * `email` - 用户邮箱
...
/// # Note
/// 此函数会删除整个用户记录，请谨慎使用。
pub async fn cleanup_oauth_user(ctx: &TestContext, email: &str) {
    // 首先删除OAuth链接
    let user_id: Option<String> =
        sqlx::query_scalar("SELECT id::text FROM account WHERE email = $1 AND realm_id = $2")
            .bind(email)
    ...
    if let Some(id) = user_id {
        cleanup_user_oauth_links(ctx, &id).await;

        // 删除用户角色
        let _ = sqlx::query("DELETE FROM user_roles WHERE user_id = $1 AND realm_id = $2")
            .bind(&id)
            .bind(&ctx._realm_id)
            .execute(&ctx.app_state.pool)
            .await;
        ...
        // 删除用户
        let result = sqlx::query("DELETE FROM account WHERE id = $1 AND realm_id = $2")
            .bind(&id)
            .bind(&ctx._realm_id)
            .execute(&ctx.app_state.pool)
            .await;
