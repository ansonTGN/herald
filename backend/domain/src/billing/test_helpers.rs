...
mod tests {
    ...
    #[test]
    fn test_plan_builder_creates_valid_plan() {
        let plan = PlanBuilder::new()
            .with_realm_id("my-realm")
            .with_name("pro")
            .with_price(2500)
            .build();

        assert_eq!(plan.realm_id, "my-realm");
        assert_eq!(plan.name, "pro");
        assert_eq!(plan.title, "pro");
        assert_eq!(plan.price, 2500);
    }

    #[test]
    fn test_subscription_builder_creates_valid_subscription() {
        ...
        assert_eq!(sub.realm_id, "my-realm");
        assert_eq!(sub.status, SubscriptionStatus::Trialing);
        assert_eq!(sub.tier, SubscriptionTier::Professional);
    }

    #[test]
    fn test_payment_event_builder_creates_valid_event() {
        let event = PaymentEventBuilder::new()
            .with_realm_id("my-realm")
            .with_external_event_id("evt_123")
            .with_payment_provider("creem")
            .build();

        assert_eq!(event.realm_id, "my-realm");
    ...
    fn test_convenience_functions() {
        let sub = test_subscription("test-realm");
        assert_eq!(sub.realm_id, "test-realm");

        let event = test_payment_event("test-realm", "evt_test");
        assert_eq!(event.realm_id, "test-realm");
        assert_eq!(event.external_event_id, "evt_test");
        assert_eq!(event.payment_provider, "creem");

        let plan = test_plan("test-realm", "pro");
