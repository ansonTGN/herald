...
mod tests {
    ...
    #[tokio::test]
    async fn test_create_points_package_success() {
        ...
        assert_eq!(package.points, 500);
        assert_eq!(package.price, 2999);
    }
    ...
    #[tokio::test]
    async fn test_update_points_package_success() {
        struct UpdateMockRepository {
            existing_package: Option<PointsPackage>,
        }

        impl PointsPackageRepository for UpdateMockRepository {
            async fn find_points_package_by_id(
                &self,
                _realm_id: &str,
                _package_id: Uuid,
            ) -> Result<Option<PointsPackage>, CoreError> {
                Ok(self.existing_package.clone())
            }
        ...
        let repo = Arc::new(UpdateMockRepository {
            existing_package: Some(existing),
        });
        let service = PointsPackageService::new(repo);
    ...
    #[tokio::test]
    async fn test_delete_points_package_success() {
        struct DeleteMockRepository;

        impl PointsPackageRepository for DeleteMockRepository {
            async fn find_points_package_by_id(
                &self,
                _realm_id: &str,
                _package_id: Uuid,
        ...
        let repo = Arc::new(DeleteMockRepository);
        let service = PointsPackageService::new(repo);
    ...
    #[tokio::test]
    async fn test_add_payment_provider_mapping_success() {
        struct AddMappingMockRepository;

        impl PointsPackageRepository for AddMappingMockRepository {
            async fn list_payment_provider_mappings(
                &self,
                _package_id: Uuid,
            ) -> Result<Vec<PointsPackagePaymentProvider>, CoreError> {
                Ok(vec![]) // No existing mappings
            }
            ...
            async fn create_payment_provider_mapping(
                ...
                Ok(PointsPackagePaymentProvider {
                    id: Uuid::now_v7(),
                    points_package_id: input.points_package_id,
        ...
        let repo = Arc::new(AddMappingMockRepository);
        let service = PointsPackageService::new(repo);

        let input = CreatePaymentProviderMappingInput {
            points_package_id: Uuid::now_v7(),
            payment_provider: "stripe".to_string(),
            external_product_id: Some("prod_123".to_string()),
            enabled: true,
        };
        ...
        assert_eq!(mapping.payment_provider, "stripe");
        assert_eq!(mapping.external_product_id, Some("prod_123".to_string()));
    ...
    #[tokio::test]
    async fn test_list_payment_provider_mappings() {
        struct ListMappingsMockRepository;

        impl PointsPackageRepository for ListMappingsMockRepository {
            async fn list_payment_provider_mappings(
                &self,
                _package_id: Uuid,
            ) -> Result<Vec<PointsPackagePaymentProvider>, CoreError> {
        ...
        let repo = Arc::new(ListMappingsMockRepository);
        let service = PointsPackageService::new(repo);

        let result = service.list_payment_provider_mappings(Uuid::now_v7()).await;

        assert!(result.is_ok());
        let mappings = result.unwrap();
        assert_eq!(mappings.len(), 2);
        assert_eq!(mappings[0].payment_provider, "stripe");
        assert_eq!(mappings[1].payment_provider, "wechat");
    }

    #[tokio::test]
    async fn test_remove_payment_provider_mapping() {
        struct RemoveMappingMockRepository;

        impl PointsPackageRepository for RemoveMappingMockRepository {
            async fn delete_payment_provider_mapping(
                &self,
                _mapping_id: Uuid,
            ) -> Result<(), CoreError> {
        ...
        let repo = Arc::new(RemoveMappingMockRepository);
        let service = PointsPackageService::new(repo);

        let result = service
            .remove_payment_provider_mapping(Uuid::now_v7())
            .await;
    ...
    #[tokio::test]
    async fn test_list_points_packages() {
        struct ListPackagesMockRepository;

        impl PointsPackageRepository for ListPackagesMockRepository {
            async fn list_points_packages(
                &self,
                _realm_id: &str,
                _enabled_only: bool,
        ...
        let repo = Arc::new(ListPackagesMockRepository);
        let service = PointsPackageService::new(repo);

        let result = service.list_points_packages("test-realm", true).await;

        assert!(result.is_ok());
        let packages = result.unwrap();
        assert_eq!(packages.len(), 2);
        assert_eq!(packages[0].name, "package1");
        assert_eq!(packages[1].name, "package2");
    }
}
