...
| crate | 当前职责 |
| --- | --- |
| `api/` | Axum router、HTTP handler、DTO、认证与 OpenAPI 导出 |
| `core/` | 共享领域模型、业务逻辑、trait、错误类型 |
| `sdk/` | 对外 SDK / 客户端能力 |
| `integration-tests/` | SDK 集成测试 |
| `test-support/` | 测试环境支持、schema 隔离与测试工具 |

### 2.2 依赖方向

- `core/` 是共享业务核心，不应反向依赖上层 HTTP 或测试逻辑。
- `api/` 负责对外 HTTP 暴露、认证与路由装配，可依赖 `core/`。
- `sdk/` 与 `integration-tests/` 围绕对外调用与验证组织，不承担 API 进程启动职责。
...
- 跨 crate 共享的协议类型优先放在 `core/`。
- 共享错误优先使用项目内稳定错误类型，不向客户端直接暴露底层错误对象。
- 新增 `core/` 领域错误时，不要继续扩大对 HTTP 类型的依赖；HTTP 状态码和响应 body 优先在 `api/` 层处理。
...
- Handler 负责参数提取、认证接入、调用应用/领域服务，以及 HTTP 错误转换。
- 权限校验的稳定真相优先放在 `core/` 的 service / policy / domain 层；不要把“handler 看得见权限检查”当作唯一正确形态。
- Handler 可以保留轻量权限前置校验，但这类校验默认视为入口防线或协议约束，不能替代 `core/` 层的最终授权判断。
- 复杂业务逻辑、跨入口复用的授权规则和租户边界规则应下沉到 `core/` 或相邻业务模块，避免把 handler 演变成权限与业务编排中心。
...
- OpenAPI 路径占位符继续使用 camelCase，例如 `realmId`、`userId`，保持前端 SDK 生成稳定。
...
- 新增或重构权限逻辑时，优先补齐 `core/` 层服务/策略中的稳定约束；只有当接口属于明显的协议入口、回调入口或需要更早失败时，才在 handler 追加薄入口校验。
