...
补充事实：
- `npm run build` 会先执行 `npm run generate-api`，再执行 `vite build && tsc --noEmit`。
- OpenAPI 导出与前端 SDK 生成以 `generate-api` 脚本链路为准。
...
当前前端是多租户管理台，不再使用旧的 `workspaceId` 模板命名。

稳定事实：
- 路由真相以 `frontend/src/routes/` 和 `frontend/src/routeTree.gen.ts` 为准。
- 新页面优先遵循当前文件路由结构和现有路由分组方式。
- `realmId` 由 URL 驱动，新增 API 调用与页面跳转必须保持这一前提一致。
- 不要把历史文档里的旧后台路径或参考仓库路由结构当作默认事实。
