...
export function userListOptions(realmId: string) {
  return queryOptions({
    queryKey: ['users', 'list', realmId],
    queryFn: async () => handleApiResponse(await listUsers({ path: { realmId } })),
  })
}
```
...
await handleApiResponse(
  await createClientApp({ path: { realmId }, body: payload }),
)
```
