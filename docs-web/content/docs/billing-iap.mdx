...
description: "A hands-on IAP integration guide for Realm Admins. Follow it end to end and your mobile app users will be able to buy subscriptions and credit packs through App Store and Google Play in-app purchases."
---

A hands-on IAP integration guide for Realm Admins. Follow it end to end and your mobile app users will be able to buy subscriptions and credit packs through App Store and Google Play in-app purchases.
...
Non-consumables and non-renewing subscriptions are not supported by Herald's fulfillment model today.
...
3. Fill the dialog:
   ...
   - **Billing Type**: `Recurring` for subscriptions, `OneTime` for consumable credit packs
   ...
   - **Validity Days** (one_time only): credit validity; 0 or empty means never expires
4. Submit
...
- **Google**: looks up the real state through the Developer API (`subscriptionsv2.get` / `products.get`). User binding comes from `obfuscatedExternalAccountId` — set it to the Herald user ID. On success Herald immediately acknowledges the subscription or consumes the consumable **inside the fulfillment transaction**. Miss the 3-day acknowledge window and Google silently refunds the purchase, so this is deliberately not deferred.
...
5. For a subscription, wait for a renewal (sandbox renewals run on an accelerated clock) or trigger a refund, and confirm the state change lands in **Subscription History**
