...
Passwords are phishable and reused. TOTP improves on that but still asks the user to type a code, and the code can be relayed in a real-time phishing attack. Passkey is a public-key credential that stays on the user's device: the private key never leaves it, the assertion is bound to the origin (RP_ID), and the browser mediates the ceremony — so a phishing page on a look-alike domain cannot produce a valid assertion.

Herald layers Passkey on top of the same building blocks TOTP already uses: Realm-level `enabled` / `force_enabled` config, the temp-session / session-issuance flow, audit, and rate limiting. Passkey does not replace TOTP — it is an additional option, and TOTP remains one of the fallbacks.
...
**First factor (passwordless).** On the login page, the browser auto-fills any discoverable credential the user has registered on this browser (conditional UI) when the username field is focused, or the user clicks *Use Passkey*. The server issues a fresh assertion challenge, the device signs it, and on success a Session is created directly — no password is ever entered.

**Second factor.** The user enters email and password. After the password verifies, if the user has a registered Passkey, Herald offers a Passkey verification step alongside TOTP. The user completes the ceremony and a Session is created. This mode parallels the existing TOTP second-factor flow.

In both modes the outcome on success is the same Session issuance path TOTP uses.
...
- Cross-Realm Passkey sharing (credentials are Realm-isolated).
- Enterprise attestation verification and FIDO MDS lookups.
- An admin remotely deleting or locking a single user's Passkey (reset the user's auth methods instead).
- A "passwordless-only" mode that disables password / TOTP.
- A Passkey-specific recovery code (TOTP backup codes remain the shared recovery mechanism).

## Related docs

- [Passkey PRD](https://github.com/timzaak/cas-2/blob/main/docs/prd/auth/passkey.md) — product scope, business rules, and acceptance goals
- [Passkey user stories](https://github.com/timzaak/cas-2/blob/main/docs/user-stories/auth/passkey.md) — US-PK-001..010 acceptance scenarios
