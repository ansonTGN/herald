...
The old model had three layers of local catalog: Product → SubscriptionPlan → Subscription. Admins created products and plans in Herald, configured payment mappings, then assigned them to Client Apps. The problem is that Stripe/Creem also have their own Product/Price/Subscription, and the two catalogs had to be kept in sync by hand — over time they drifted.

The new model hands the commercial catalog to the payment provider. Herald no longer offers CRUD for Product and SubscriptionPlan. Instead, a mapping table (Entitlement Mapping) records "which provider product corresponds to which Herald entitlement."

The credits side changed too. The old credits ledger was "one global balance per user," mixing all credits together. The new model uses Credit Accounts to split credits into multiple isolated pools, settled and consumed per app group. The Credit Account is the common anchor for several concepts: mappings are attached to it, subscriptions are bound to it, and credits are granted into its pools.
...
1. **Admin manual sync**: calls the provider's Product API, pulls all products, and auto-creates or updates mappings. Newly created mappings are first bound to this Realm's registration receiving pool; afterwards the admin can move them to another Credit Account.
2. **Webhook incremental update**: when a payment event fires, info is extracted from the webhook payload to update the cache.
...
Projection fields include `realm_id`, `user_id`, `entitlement_key`, `bucket_id`, `status`, `current_period_start/end`, `provider_metadata`, and so on. When a subscription is established, it is bound to the Credit Account that the purchased Entitlement Mapping belonged to at that moment. Later renewals, upgrades, downgrades, cancellations, and refunds all return to the same pool — they never leak into another Credit Account. The old `plan_id`, `tier`, and `billing_period` fields have been removed; when you need that information, derive it from `entitlement_key` or `provider_metadata`.
...
The following features were removed in the June 2026 refactor and no longer exist:

- Local Product CRUD (admin page and API)
- Local SubscriptionPlan CRUD (admin page and API)
- Plan → Payment Provider Mapping configuration
- Plan → Client App assignment
- Join tables: `subscription_plan_payment_provider`, `client_app_subscription_plan`, `points_plan_configs`
- The "unanchored mapping → not purchasable" intermediate state: `bucket_id` is NOT NULL on mappings, subscriptions, and payment attempts. A product either belongs to some Credit Account or is not purchasable — there is no dangling state.

If you see references to "plan_id", "Product management", or "Subscription Plan" in code or docs, they are obsolete.

## Related docs

- [Credit Account user stories](https://github.com/timzaak/cas-2/blob/main/docs/user-stories/billing/credit-bucket.md) — acceptance scenarios for the Credit Account catalog, coverage sets, purchase-to-pool, cross-pool consumption, and the subscription lifecycle
- [Stripe integration guide](billing-stripe-payment) — Stripe provider configuration and webhook handling
- [Creem integration guide](billing-creem-payment) — Creem provider configuration and webhook handling
- [Invoice management](billing-invoice) — Invoice creation, issuance, and PDF generation
