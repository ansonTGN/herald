...
The frontend owns token storage and sends `Authorization: Bearer` on every request. It handles four things: detect auth state, hold tokens after login, attach the Bearer header, and refresh on 401.
