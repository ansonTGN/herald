...
## How the certificate-abuse gate works
...
The shared secret between Herald and the proxy is `[custom_domain].ask_key`; the proxy passes it in the `X-Herald-Ask-Key` header. See [Deployment](/docs/deployment) for reverse-proxy / on-demand TLS setup.
...
- [White-label](/docs/ui-custom) — brands the page; custom domain brands the URL. White-label uses a draft/publish lifecycle; custom domain is a single save.
