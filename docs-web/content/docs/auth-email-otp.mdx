...
Send is Turnstile-gated when the Client App has `turnstile_enabled` on. The frontend renders the widget, solves the challenge, and sends the resulting `turnstileToken` in the send request body. Verify does not require a fresh Turnstile token. See [Custom User UI](/docs/custom-user-ui) for how Turnstile is configured per Client App.

## Related docs

- [Custom User UI](/docs/custom-user-ui) — the self-built frontend path, including how to render the Turnstile widget and store the resulting tokens
- [White-label](/docs/ui-custom) — branding the hosted auth pages where the email-code entry also appears
