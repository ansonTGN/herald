...
- **Redis**：token 存储（access/refresh token 家族）、权限缓存、限流（Redis Functions）、幂等键。用自建的 `RedisConnectionManager` 包装，测试时自动切到 DB 1 隔离数据。
...
HTTP Request
  → Axum Router（路由匹配）
  → inject_identity 中间件（从 Authorization: Bearer token 解析用户身份）
...
| `realm` | 租户管理（Admin Realm 管理员手动创建；另有托管在 admin realm 的公共自助开通入口 `/api/auth/admin/signup`，未登录访客可借此开通一个新 realm 并成为其 admin） |
...
| `api-auth` | 注册、登录、密码重置、邮箱验证 | Bearer token（公开端点 + 签发 token） |
| `api-admin` | 用户 CRUD、角色管理、权限定义管理 | Bearer token（FirstParty）+ inject_identity |
...
| `api-points` | 积分余额、交易历史、消费、充值 | Bearer token 或 API Key |
