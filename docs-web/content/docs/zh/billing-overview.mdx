...
| 字段 | 说明 |
|------|------|
| `payment_provider` | 支付方名称（stripe、creem、apple、google） |
...
1. **管理员手动同步**：调用支付方 Product API，拉取所有商品，自动创建或更新映射。新建的映射先绑定到本 Realm 的注册接收池，之后管理员可以把它挪到别的积分账户。适用于 Stripe 和 Creem。
2. **管理员手工创建**：IAP 支付方（App Store / Google Play）没有商品目录同步，管理员直接用商店商品 ID 建映射。
...
Stripe、Creem 是发起式平台，通过 PaymentAttempt 统一管理支付过程。IAP 同样经 PaymentAttempt 入账，但履约由移动 App 提交购买凭证（Apple `jwsRepresentation` / Google `purchaseToken`）触发，而不是 checkout webhook——平台通知（Apple）和定时轮询（Google）只负责之后的生命周期。详见 [IAP 对接指南](/docs/billing-iap)。
