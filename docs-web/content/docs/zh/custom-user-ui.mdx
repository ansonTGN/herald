...
3. **已启用**——该 Client App 必须处于启用状态。后续禁用会使它的浏览器 token 家族失效。

Realm 还必须启用注册和登录。Turnstile 和按 IP 的限速对所有公开端点（`/login`、`/register`、邮箱验证、密码重置）生效，因此前端在提交这些表单前必须先完成 Turnstile 挑战。
...
- **公开端点的 Turnstile + 限速。** `/login`、`/register`、邮箱验证、密码重置均受 Turnstile 保护并按 IP 限速。

## 相关文档

- [Custom User UI PRD](https://github.com/timzaak/cas-2/blob/main/docs/prd/integration/custom-user-ui.md)——产品范围、业务规则、验收目标
- [Custom User UI 用户故事](https://github.com/timzaak/cas-2/blob/main/docs/user-stories/integration/custom-user-ui.md)——验收场景
