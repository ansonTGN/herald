...
当反代层（Caddy）被要求通过 on-demand TLS 为某个 hostname 签发证书时，它会先问 Herald：「这个 hostname 注册并上线了吗？」Herald 用一个布尔值回答——`authorized: true` 或 `404`，不泄露任何 Realm 身份或其他细节。

- 已配置且启用的 hostname → `authorized: true` → 反代继续 ACME 挑战。
- 未注册的 hostname → `404` → 反代拒绝签发。指向 Herald 的钓鱼域名拿不到证书。

授权门控只检查「已配置且启用」。CNAME 校验通过和 TLS 就绪是管理面板的展示字段，不参与授权决策——否则授权和签发会形成循环依赖（`tls_ready=false` → 拒绝签发 → `tls_ready` 永远不会变 `true`）。

Herald 和反代之间的共享密钥是 `[custom_domain].ask_key`；反代通过 `X-Herald-Ask-Key` 头传入。反代 / on-demand TLS 的接法见 [部署](/docs/deployment)。
...
flowchart LR
    ...
    subgraph Proxy[反代 / Caddy]
        TLS[On-Demand TLS<br/>签发前先问]
    end

    UI -->|保存写入 hostname + config| CFG
...
管理员通过管理端点保存 hostname，同时写入配置行和域名注册映射。反代在签发任何证书前先问授权端点；只有已映射、已启用的 hostname 才会被授权。
