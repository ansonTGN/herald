...
这是 Standard White-label 能力，不是完整的主题系统。范围对标 Auth0 / Clerk / WorkOS 所说的 "basic branding"：通过表单填入资产 URL 和颜色选择，应用到 Herald 托管的 auth 页面。不提供自定义 HTML/CSS、自定义域名、资产上传。
...
Herald 里每个 Realm 默认都用同一套 auth 页面：「Herald」文字 logo、固定渐变背景、Herald 的按钮主色。对把 Herald 当作身份提供方的租户来说，这个默认外观在最重要的信任时刻——用户输入密码或确认 OAuth 授权——破坏了品牌连续性。

White-label 把这个界面交给 Realm 管理员。指向一个 logo URL、选一个主色、贴一段背景、发布，之后该 Realm 下所有 auth 流页面都带上租户品牌。即使主色对比度不达标，最终决定权也在管理员手上。
...
Realm 管理员在 *Settings → White-label* 配置品牌化。这个 Tab 和现有的 General / TOTP / Passkey / Registration / Email 等 Tab 并列，复用同一套 Realm-config 存储，所以权限模型与 Settings 其他部分一致：读取要 `settings.view`，保存要 `settings.manage`。
...
auth 流之外的页面不做品牌化。legal 页、用户中心、管理后台本身保留 Herald 外观，即使技术上共享部分样式基础设施。这是范围决策，不是技术限制——理由见 PRD 的 out-of-scope 章节。
...
- 自定义 HTML 或 CSS 模板（Full Custom）。对 Standard 级别而言 XSS / CSP 面太大。
- 自定义子域 / 自定义域名。属于更高层级。
- 资产上传与存储。本期 logo 和背景都是 URL 引用；如果普遍缺图床的租户变多，上传是可能的扩展。
- 资产 / 内容安全审核流水线。外部 URL 由租户负责。
- 多语言品牌文案。i18n 是独立关注点，另有 PRD 覆盖。
- 暗色模式的品牌值。本期只对 light 主题做品牌化。
- 非 auth 页面（legal、用户中心、管理后台）的品牌化。

## 相关文档

- [White-label PRD](https://github.com/timzaak/cas-2/blob/main/docs/prd/core/ui-custom.md) — 产品范围、业务规则、验收目标
- [White-label 用户故事](https://github.com/timzaak/cas-2/blob/main/docs/user-stories/core/white-label.md) — US-WL-001..004 验收场景
