...
description: "面向 Realm Admin 的 IAP 对接操作指南。跟着做完，你的移动 App 用户就能通过 App Store 和 Google Play 内购完成订阅和积分包购买。"
---

面向 Realm Admin 的 IAP 对接操作指南。跟着做完，你的移动 App 用户就能通过 App Store 和 Google Play 内购完成订阅和积分包购买。
...
非消耗型买断和非续期订阅目前不在 Herald 履约模型支持范围内。
...
- **Google**：调 Developer API（`subscriptionsv2.get` / `products.get`）回查真实状态。用户绑定来自 `obfuscatedExternalAccountId`——同样设成 Herald 用户 ID。校验成功后 Herald 在**履约事务内**立即 acknowledge（订阅）或 consume（消耗型）。Google 有 3 天确认硬截止，超期会静默退款，所以这一步刻意不做异步延迟。
...
5. 订阅类商品等一次续费（沙盒续费按加速时钟走）或发起一笔退款，确认状态变化出现在 **Subscription History** 里
