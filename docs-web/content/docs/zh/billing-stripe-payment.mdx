...
一个产品可以挂多个货币的 Price——在 Stripe Dashboard 为每个货币加一个 Price（比如 $9.99/月 和 €9.99/月），重新同步后每个 Price 各成一行映射。购买页会按货币分组并高亮用户的偏好货币。Realm/用户两级货币偏好和 api-ext 按货币解析接口见[多货币定价指南](/docs/zh/billing-multi-currency)。
...
产品带多个货币的 Price 时，Herald 购买页按货币分组，并高亮用户的生效偏好货币（个人偏好，否则 Realm 默认）。用户先选货币，再在组内选计费周期，支付尝试指向选中的那一行——扣款货币始终与展示一致。
