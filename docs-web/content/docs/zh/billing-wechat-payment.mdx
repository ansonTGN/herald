...
有一个需要知道的缺口：创建弹窗没有价格字段，映射 API 也不接受价格。订单金额从映射的 `provider_product_info` 读取：`price` 是以分为单位的整数（`1000` 即 10.00 元），`currency` 通常为 `cny`。这段 JSON 目前需要直接写进数据库：
...
没配价格的映射也能卖——订单会带 1 分的哨兵金额以满足支付尝试的约束。测试订单显示 0.01 元，就是这个原因。
...
2. 在 **Entitlement Mappings** 确认映射行存在，计费类型正确、`provider_product_info` 里有价格、（会员类）服务天数已设置
...
映射的 `provider_product_info` 里没有 `price`。Herald 发送 1 分哨兵金额让支付尝试行通过 `amount > 0` 约束。补上价格（见第 2 步）后重新下单——旧尝试保持原金额。
...
- [ ] `provider_product_info` 已设置价格与币种（最小单位整数；用测试订单金额验证）
