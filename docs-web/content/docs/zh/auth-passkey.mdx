...
密码可被钓鱼、且常被复用。TOTP 好一些，但仍然要用户手输验证码，验证码在实时钓鱼攻击里会被中继转发。Passkey 是留在用户设备上的公钥凭证：私钥不离开设备，断言绑定到 origin（RP_ID），ceremony 由浏览器中介——所以一个长得像的钓鱼域名页面无法产出有效断言。

Herald 把 Passkey 叠加在 TOTP 已经用的同一套积木上：Realm 级的 `enabled` / `force_enabled` 配置、临时会话 / 会话发放流程、审计、速率限制。Passkey 不替换 TOTP，它是一个额外选项，TOTP 仍然是回退方式之一。

## 两种登录模式

一个 Passkey 凭证同时服务两种模式。这是有意的设计：注册了 Passkey 的用户不用想"这是登录凭证还是 2FA 凭证"——它两者都是。

**第一因素（无密码）。** 登录页上，用户名输入框聚焦时浏览器会自动填充该浏览器上已注册的可发现凭证（conditional UI），或者用户点 *Use Passkey*。服务器签发一次性断言 challenge，设备签名，成功后直接创建 Session——全程不输密码。

**第二因素。** 用户输入邮箱和密码。密码验证通过后，如果用户已注册 Passkey，Herald 在 TOTP 旁并列提供一个 Passkey 验证步骤。用户完成 ceremony，创建 Session。这个模式和现有 TOTP 第二因素流程平行。

两种模式成功后走的是同一条 Session 发放路径，和 TOTP 一致。
...
- 跨 Realm 共享 Passkey（凭证按 Realm 隔离）。
- 企业级 attestation 验证和 FIDO MDS 查询。
- 管理员远程删除或锁定单个用户的 Passkey（改为重置该用户的认证方式）。
- 禁用密码 / TOTP 的"纯无密码"模式。
- Passkey 专属恢复码（TOTP 备份恢复码作为通用恢复机制保留）。

## 相关文档

- [Passkey PRD](https://github.com/timzaak/cas-2/blob/main/docs/prd/auth/passkey.md) — 产品范围、业务规则、验收目标
- [Passkey 用户故事](https://github.com/timzaak/cas-2/blob/main/docs/user-stories/auth/passkey.md) — US-PK-001..010 验收场景
