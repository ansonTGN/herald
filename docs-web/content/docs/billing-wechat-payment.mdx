...
There is a gap you should know about: the create dialog has no price field, and the mapping API does not accept one either. The order amount is read from the mapping's `provider_product_info`: `price` as an integer in the smallest currency unit (fen, so `1000` = 10.00 CNY) and `currency` (typically `cny`). Today that JSON is written directly against the database:

```sql
UPDATE provider_entitlement_mappings
SET provider_product_info = '{"name":"Membership 30d","price":2000,"currency":"cny"}'::jsonb
WHERE realm_id = '<realm>' AND payment_provider = 'wechat' AND external_product_id = 'membership-30d';
```

A mapping without a price still sells: the order goes out with a 1-fen sentinel amount to satisfy the payment attempt constraint. If a test order shows 0.01 CNY, this is why.
...
2. On **Entitlement Mappings**, confirm the mapping row exists with the right billing type, price in `provider_product_info`, and (for memberships) service duration days
...
The mapping has no `price` in `provider_product_info`. Herald sends a 1-fen sentinel so the attempt row passes its `amount > 0` constraint. Set the price (see Step 2) and place a new order — the old attempt keeps its original amount.
...
- [ ] Price and currency set in `provider_product_info` (integer smallest unit; verified by a test order's amount)
