...
When the product carries Prices in several currencies, the Herald purchase page groups them by currency and highlights the user's effective preferred currency (personal preference, else the realm default). The user picks a currency, picks a billing period inside that group, and the payment attempt targets that exact row — the charge currency always matches what was shown.
