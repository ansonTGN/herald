...
Run the Credit-Note-related scenarios directly:
...
- `credit_note.voided` rolls back the amount
- duplicate `credit_note.voided` is idempotent
- a voided event arriving before the created event triggers re-delivery
- void is rejected with 409 when an active Credit Note exists
- void succeeds when only voided Credit Notes exist
...
- refund summary render gating (stripe/manual shown, creem not)
- dual-track Credit Note list
- strikethrough on voided rows
- Record Refund button gating
- user-role trimming

### Demo

Demo scripts live in `demo/e2e/`:
...
Before running, start the demo environment and make sure the `STRIPE_WEBHOOK_SECRET` in `demo/.env.demo` matches the webhook secret in the realm seed.
...
- [Invoice main flow](billing-invoice)
- [Stripe payment and webhooks](billing-stripe-payment)
