---
title: Architecture
description: "Rust backend (hexagonal architecture) + React frontend (TanStack stack), deployed as a single process."
---
...
- **Redis**: token storage (access/refresh token families), permission caching, rate limiting (Redis Functions), idempotency keys. Wrapped in a custom `RedisConnectionManager` that automatically switches to DB 1 during tests for data isolation.
...
HTTP Request
  ...
  → inject_identity middleware (resolves user identity from the Authorization: Bearer token)
...
| `realm` | Tenant management (manual creation by Admin Realm admins, plus a public self-service signup entry hosted on the admin realm — `/api/auth/admin/signup` — that lets an unauthenticated visitor provision a new realm and become its admin) |
...
Payment channel clients live in separate crates (`infra-creem`, `infra-stripe`, `infra-shopify`) rather than inside the main `infra` crate. The reason is to avoid pulling in provider SDKs you don't need — if you only use Stripe, the Shopify SDK won't get compiled.
...
| `api-auth` | Registration, login, password reset, email verification | Bearer token (public endpoints + token issuance) |
| `api-admin` | User CRUD, role management, permission definition management | Bearer token (FirstParty) + inject_identity |
...
| `api-points` | Points balance, transaction history, consumption, top-up | Bearer token or API Key |
