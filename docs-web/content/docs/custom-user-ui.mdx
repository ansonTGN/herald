...
3. **Enabled** — the Client App must be enabled. Disabling it later invalidates its browser token families.

The realm must also have registration and login enabled. Turnstile and per-IP rate-limiting apply to all public endpoints (`/login`, `/register`, email-verify, password-reset), so the frontend must complete the Turnstile challenge before submitting those forms.
...
- **Turnstile + rate-limiting on public endpoints.** `/login`, `/register`, email-verify, and password-reset are Turnstile-gated and rate-limited per IP.

## Related docs

- [Custom User UI PRD](https://github.com/timzaak/cas-2/blob/main/docs/prd/integration/custom-user-ui.md) — product scope, business rules, acceptance goals
- [Custom User UI user stories](https://github.com/timzaak/cas-2/blob/main/docs/user-stories/integration/custom-user-ui.md) — acceptance scenarios
