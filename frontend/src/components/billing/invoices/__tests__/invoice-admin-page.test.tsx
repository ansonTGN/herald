...
describe('InvoiceAdminPage', () => {
  ...
  describe('table rendering', () => {
    it('renders table with invoice data showing expected columns', async () => {
  ...
  describe('loading state', () => {
    it('shows loading message while fetching', async () => {
      ...
      server.use(
        http.get(`${BASE_URL}/api/bill/${REALM_ID}/invoices`, async () => {
  ...
  describe('empty state', () => {
    it('shows "No invoices yet." when API returns empty list', async () => {
      server.use(
  ...
  describe('error state', () => {
    it('shows error message when API fails', async () => {
      ...
      // invoiceListQueryOptions has retry: 1, so we need to wait longer
      // for the retry to complete before the error state renders
      await waitFor(
        () => {
          expect(screen.queryByText('Loading invoices...')).not.toBeInTheDocument()
        },
      ...
      // Error message should be displayed by DataTable
      expect(screen.getByTestId('invoice-table')).toHaveTextContent(/Error/)
    })
  })
  ...
  describe('status filter', () => {
    it('selecting "draft" filters by status query param', async () => {
      ...
      server.use(
        http.get(`${BASE_URL}/api/bill/${REALM_ID}/invoices`, ({ request }) => {
          ...
          const invoices = [makeInvoice({ id: 'inv-1', status: 'draft', invoiceNumber: 'INV-D1' })]
          return HttpResponse.json(makeListResponse(invoices))
        })
      )
      ...
      const statusFilter = screen.getByTestId('invoice-status-filter')
  ...
  describe('source filter', () => {
    it('selecting "user_application" sends correct query param', async () => {
      const user = userEvent.setup()
      ...
      server.use(
        http.get(`${BASE_URL}/api/bill/${REALM_ID}/invoices`, ({ request }) => {
          const url = new URL(request.url)
          capturedSource = url.searchParams.get('source')
          const invoices = [
            makeInvoice({ id: 'inv-2', source: 'user_application', invoiceNumber: 'INV-UA1' }),
          ]
          return HttpResponse.json(makeListResponse(invoices))
        })
      )
      ...
      // Open source filter and select "Application"
      const sourceFilter = screen.getByTestId('invoice-source-filter')
      await user.click(sourceFilter)

      const appOption = await screen.findByRole('option', { name: 'Application' })
      await user.click(appOption)

      await waitFor(() => {
        expect(capturedSource).toBe('user_application')
      })
    })
  })
