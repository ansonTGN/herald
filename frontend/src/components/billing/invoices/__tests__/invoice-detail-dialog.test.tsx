...
describe('InvoiceDetailDialog', () => {
  ...
  describe('dialog rendering', () => {
    it('opens and renders invoice number and status badge', async () => {
      setupDetailDialog({ status: 'issued', history: [] })

      // Wait for data to load by checking for a data-dependent element
      await waitFor(() => {
        expect(screen.getByTestId('invoice-seller-info')).toBeInTheDocument()
      })

      // Verify invoice number and status badge in the dialog title
      expect(screen.getByText(/INV-001/)).toBeInTheDocument()
      expect(screen.getByText('Issued')).toBeInTheDocument()
    })

    it('does not render when open is false', () => {
      setupDetailDialog({}, { open: false })

      expect(screen.queryByTestId('invoice-detail-dialog')).not.toBeInTheDocument()
    })
  })

  // ==================== Seller / Buyer Two-Column Layout ====================

  describe('seller and buyer info', () => {
    it('displays seller info on the left side', async () => {
      setupDetailDialog()
      ...
      const seller = within(screen.getByTestId('invoice-seller-info'))
      expect(seller.getByText('Seller')).toBeInTheDocument()
      expect(seller.getByText('Test Seller')).toBeInTheDocument()
      expect(seller.getByText('seller@test.com')).toBeInTheDocument()
      expect(seller.getByText('456 Seller Ave')).toBeInTheDocument()
      expect(seller.getByText('444-555-6666')).toBeInTheDocument()
    })

    it('displays buyer info on the right side', async () => {
      setupDetailDialog()

      await waitFor(() => {
        expect(screen.getByTestId('invoice-buyer-info')).toBeInTheDocument()
      })

      const buyer = within(screen.getByTestId('invoice-buyer-info'))
      expect(buyer.getByText('Buyer')).toBeInTheDocument()
      expect(buyer.getByText('Test Buyer')).toBeInTheDocument()
      expect(buyer.getByText('buyer@test.com')).toBeInTheDocument()
      expect(buyer.getByText('123 Buyer St')).toBeInTheDocument()
      expect(buyer.getByText('111-222-3333')).toBeInTheDocument()
    })
  })
  ...
  describe('line items table', () => {
    it('renders all line items with name, quantity, unit price, and subtotal', async () => {
      setupDetailDialog()
      ...
      const section = within(screen.getByTestId('invoice-line-items-section'))

      // Table headers
      expect(section.getByText('Line Items')).toBeInTheDocument()
      expect(section.getByText('Name')).toBeInTheDocument()
      expect(section.getByText('Qty')).toBeInTheDocument()
      expect(section.getByText('Unit Price')).toBeInTheDocument()
      // "Subtotal" appears as both a table header and amount breakdown label,
      // so scope to the line items section
      expect(section.getAllByText('Subtotal')).toHaveLength(1)

      // Line item 1: Service A
      expect(section.getByText('Service A')).toBeInTheDocument()
      expect(section.getByText('Service A description')).toBeInTheDocument()

      // Line item 2: Service B
      expect(section.getByText('Service B')).toBeInTheDocument()
    })
  })

  // ==================== Amount Breakdown ====================

  describe('amount breakdown', () => {
    it('renders subtotal, discount, tax, shipping, and total formatted correctly', async () => {
      setupDetailDialog()

      await waitFor(() => {
        expect(screen.getByTestId('invoice-amount-breakdown')).toBeInTheDocument()
      })

      const breakdown = within(screen.getByTestId('invoice-amount-breakdown'))

      // Labels
      expect(breakdown.getByText('Subtotal')).toBeInTheDocument()
      expect(breakdown.getByText('Discount (-)')).toBeInTheDocument()
      expect(breakdown.getByText('Tax (+)')).toBeInTheDocument()
      expect(breakdown.getByText('Shipping (+)')).toBeInTheDocument()
      expect(breakdown.getByText('Total')).toBeInTheDocument()
