...
import type { UserDetailResponse } from '@/lib/api-generated'
...
export function PointsWalletsPage({ realmId }: PointsWalletsPageProps) {
  ...
  const walletUserIds = useMemo(
    () => [...new Set(walletsData?.wallets.map((wallet) => wallet.userId) ?? [])],
    [walletsData?.wallets]
  )
  ...
  function renderWalletRow(wallet: NonNullable<typeof walletsData>['wallets'][number]) {
    const user = usersById.get(wallet.userId)
    const displayName = user?.nickname || user?.email || wallet.userId
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="min-w-0">
            ...
            <div className="truncate text-xs text-muted-foreground">
              {m['points.wallets_user_id_label']()}: {wallet.userId}
            </div>
          </div>
          ...
          <CardHeader>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <div className="w-full space-y-1 sm:w-80">
                ...
                <p className="text-xs text-muted-foreground">{m['points.wallets_search_help']()}</p>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            {walletsLoading ? (
              ...
              <div className="space-y-2">
                {walletsData.wallets.map((wallet) => renderWalletRow(wallet))}
              </div>
            ) : (
              <div className="text-center py-8 text-muted-foreground">
                {searchQuery ? m['points.wallets_empty_search']() : m['points.wallets_empty']()}
              </div>
            )}
          </CardContent>
        </Card>
